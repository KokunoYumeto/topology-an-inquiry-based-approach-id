#!/usr/bin/env python3
"""Build the fail-closed cumulative Chapters 1-7 source and artifact manifest."""
...
from qa_chapter07_companion import (
...
LANE = ROOT.parent
OUTPUT = ROOT / "qa" / "CHAPTER07_SOURCE_MANIFEST.json"
AUTHORITY_SOURCE = (
    LANE
    / "authority/gvsu-pinned/topology-0c2d8f614ef87aa00de373f3418146c2f1d13bb9/source"
)
PRIOR_MANIFEST = "qa/CHAPTER06_SOURCE_MANIFEST.json"
PRIOR_MANIFEST_IDENTITY = {
    "bytes": 32_105,
    "sha256": "f6e8b1c1f331c7bb8c895ee082cd25db08c349bc60ad381b2842a4b7f9f10e44",
}
PRIOR_CHAPTERS = tuple(f"chapter_{number:02d}" for number in range(1, 7))
CHAPTER_ORDER = (*PRIOR_CHAPTERS, "chapter_07")
COMPANION_WRAPPER = "companion/chapter_07_open_balls_self_study.ptx"
COMPANION_FRAGMENTS = tuple(f"companion/{name}" for name in FRAGMENTS)
ALIAS_MAP = "backend/chapter_07_entry_aliases.csv"
BACKEND_MANIFEST = "backend/chapter_07_companion_manifest.json"
NEW_ADDITIVE = (
    COMPANION_WRAPPER,
    *COMPANION_FRAGMENTS,
    ALIAS_MAP,
...
CHAPTER_07_IMPLEMENTATION = (
    ...
    "scripts/qa_chapter07_companion.py",
    "scripts/build_chapter07_backend.py",
    "scripts/build_chapter07_source_manifest.py",
    "scripts/build_pretext_pdf_strict.py",
    "source/chapters_01_07_reader.ptx",
    "xsl/custom-latex.xsl",
)
EVOLVING_PRIOR_IMPLEMENTATION = {
    "LICENSES.md": "append-only Chapter 7 collection-rights clarification",
    "assets/o003-readable-layout.css": "cumulative reader layout evolution",
    "companion/RIGHTS.md": "append-only Chapter 7 companion rights note",
    "project.ptx": "addition of cumulative Chapters 1-7 build targets",
    "scripts/qa_source_translation.py": "Chapter 7 source topology and approved-change validation",
...
EXPECTED_PRIOR_SOURCE_FILES = 37
EXPECTED_TOTAL_SOURCE_FILES = 43
...
def validate_html_tree(
    ...
    if actual_names != sorted(expected):
        raise RuntimeError("canonical Chapter 7 HTML tree differs from its manifest")
    total_bytes = 0
    html_files = 0
...
def main() -> int:
    ...
    assert_identity("pinned Chapter 6 source manifest", PRIOR_MANIFEST_IDENTITY, identity(prior_path))
    prior = read_json(PRIOR_MANIFEST)
    require_pass(prior, "Chapter 6 cumulative source manifest")
    if prior.get("boundary") != "chapters_01_06_with_separately_licensed_self_study_companions":
        raise RuntimeError("unexpected Chapter 6 admission boundary")
    ...
    if set(prior_source_names).intersection(SOURCE_FILES):
        raise RuntimeError("Chapter 7 source closure overlaps prior chapters")

    source_qa = read_json("qa/CHAPTER07_SOURCE_QA.json")
    companion_qa = read_json("qa/CHAPTER07_COMPANION_QA.json")
    backend = read_json(BACKEND_MANIFEST)
    require_pass(source_qa, "Chapter 7 source QA")
    require_pass(companion_qa, "Chapter 7 companion QA")
    if source_qa.get("failures") != [] or companion_qa.get("failures") != []:
        raise RuntimeError("Chapter 7 source or companion QA records failures")

    source_qa_rows = {
        str(row.get("file")): require_dict(row, "Chapter 7 source-QA file")
        for row in require_list(source_qa.get("files"), "Chapter 7 source-QA files")
    }
    if set(source_qa_rows) != set(SOURCE_FILES):
        raise RuntimeError("Chapter 7 source-QA file closure changed")
    ...
    for name in SOURCE_FILES:
        ...
        assert_identity(f"Chapter 7 authority {name}", row.get("authority"), authority)
        assert_identity(f"Chapter 7 translated source {name}", row.get("translated"), translated)
    ...
    if authority_combined != EXPECTED_AUTHORITY_ORDERED_SHA256:
        raise RuntimeError("frozen Chapter 7 authority closure identity changed")
    ...
    if source_qa.get("combined_translated_sha256") != chapter_combined:
        raise RuntimeError("Chapter 7 source-QA combined translated identity is stale")

    insertion_rows = {
        row.get("key"): row
        for row in require_list(
            source_qa.get("approved_element_insertions"),
            "Chapter 7 approved insertions",
        )
    }
    if set(insertion_rows) != set(EXPECTED_INSERTIONS):
        raise RuntimeError("Chapter 7 approved insertion closure changed")
    if source_qa.get("approved_math_changes") != [
        {
            "authority": "<m>S</m>",
            "key": "sec_open_balls_exer.ptx:45",
            "translated": "<m>X</m>",
        }
    ]:
        raise RuntimeError("Chapter 7 protected-math change is not exactly S to X")
    if set(source_qa.get("approved_external_xref_targets", [])) != EXPECTED_EXTERNAL_XREFS:
        raise RuntimeError("Chapter 7 external-xref admission changed")

    companion_row = file_row(COMPANION_WRAPPER)
    companion_qa_row = file_row("qa/CHAPTER07_COMPANION_QA.json")
    source_qa_row = file_row("qa/CHAPTER07_SOURCE_QA.json")
    backend_row = file_row(BACKEND_MANIFEST)
    alias_row = file_row(ALIAS_MAP)
    assert_identity("Chapter 7 companion", companion_qa.get("companion"), companion_row)
    assert_identity("Chapter 7 source-QA receipt", companion_qa.get("source_qa"), source_qa_row)
    fragment_rows = rows_by_path(companion_qa.get("fragments"), "Chapter 7 fragments")
    expected_fragment_names = {f"companion/{name}" for name in FRAGMENTS}
    if set(fragment_rows) != expected_fragment_names:
        raise RuntimeError("Chapter 7 companion fragment closure changed")
    for relative, row in fragment_rows.items():
        assert_identity(f"Chapter 7 fragment {relative}", row, file_row(relative))

    if backend.get("schema_version") != "2.0.0" or backend.get("locale") != "id-ID":
        raise RuntimeError("Chapter 7 backend schema or locale changed")
    component = require_dict(backend.get("component"), "Chapter 7 backend component")
    assert_path("Chapter 7 backend component", component.get("path"), f"repo/{COMPANION_WRAPPER}")
    assert_identity("Chapter 7 backend component", component.get("identity"), companion_row)
    backend_alias = require_dict(component.get("entry_alias_map"), "Chapter 7 backend aliases")
    assert_path("Chapter 7 backend aliases", backend_alias.get("path"), f"repo/{ALIAS_MAP}")
    assert_identity("Chapter 7 backend aliases", backend_alias, alias_row)
    backend_source_qa = require_dict(backend.get("translated_unit_source_qa"), "backend source QA")
    assert_path("backend source QA", backend_source_qa.get("path"), "repo/qa/CHAPTER07_SOURCE_QA.json")
    ...
    if backend_source_qa.get("combined_translated_sha256") != chapter_combined:
        raise RuntimeError("backend Chapter 7 translated-source identity is stale")
    backend_companion_qa = require_dict(backend.get("companion_qa"), "backend companion QA")
    assert_path("backend companion QA", backend_companion_qa.get("path"), "repo/qa/CHAPTER07_COMPANION_QA.json")
    ...
    if entry_counts != {
        "source_prompt_guide": 40,
        "activity_or_task_guide": 16,
        "exercise_prompt_guide": 24,
        "mastery_check": 6,
        "total": 46,
    }:
        raise RuntimeError("Chapter 7 companion entry-count contract changed")
    ...
    for kind in ("statement", "hint", "answer", "solution"):
        if surface_counts.get(kind) != 46:
            raise RuntimeError(f"Chapter 7 companion lacks complete {kind} surfaces")
    for kind in ("hint", "answer", "solution"):
        if reveal_counts.get(kind) != 46:
            raise RuntimeError(f"Chapter 7 companion lacks complete {kind} reveals")
    coverage = require_dict(backend.get("coverage_contract"), "backend coverage")
    expected_coverage = {
        "source_prompt_guides": 40,
        "activity_or_task_guides": 16,
        "exercise_prompt_guides": 24,
        "mastery_checks": 6,
        "total_entries": 46,
        "statements": 46,
        "hints": 46,
        ...
        "source_correction_records": 7,
    }
    for key, expected in expected_coverage.items():
        if coverage.get(key) != expected:
            raise RuntimeError(f"Chapter 7 backend coverage is stale for {key}")
    if coverage.get("all_entries_have_statement_hint_answer_solution") is not True:
        raise RuntimeError("Chapter 7 backend complete-surface contract is false")
    ...
    if inherited_additive_paths.intersection(NEW_ADDITIVE):
        raise RuntimeError("Chapter 7 additive component duplicates a prior component")
    ...
    for relative in CHAPTER_07_IMPLEMENTATION:
        implementation_by_path[relative] = file_row(relative)

    html_manifest = read_json("qa/CHAPTER07_HTML_MANIFEST.json")
    html_run_1 = read_json("qa/CHAPTER07_HTML_MANIFEST_RUN1.json")
    html_run_2 = read_json("qa/CHAPTER07_HTML_MANIFEST_RUN2.json")
    html_qa = read_json("qa/CHAPTER07_HTML_QA.json")
    browser_qa = read_json("qa/CHAPTER07_BROWSER_QA.json")
    require_pass(html_qa, "Chapter 7 HTML QA")
    require_pass(browser_qa, "Chapter 7 browser QA")
    ...
    html_root = ROOT / "output" / "chapters01-07-html"
    html_file_count, html_files, html_bytes = validate_html_tree(html_root, html_manifest)

    pdf_run_1 = read_json("qa/CHAPTER07_PDF_RUN1_HASH.json")
    pdf_run_2 = read_json("qa/CHAPTER07_PDF_RUN2_HASH.json")
    pdf_qa = read_json("qa/CHAPTER07_PDF_VISUAL_QA.json")
    require_pass(pdf_run_1, "Chapter 7 PDF run 1")
    require_pass(pdf_run_2, "Chapter 7 PDF run 2")
    require_pass(pdf_qa, "Chapter 7 PDF visual QA")
    pdf_relative = "output/chapters01-07-pdf/chapters_01_07_reader.pdf"
    pdf_row = file_row(pdf_relative)
    assert_identity("Chapter 7 PDF run 1", pdf_run_1, pdf_row)
    assert_identity("Chapter 7 PDF run 2", pdf_run_2, pdf_row)
    pdf_visual = require_dict(pdf_qa.get("pdf"), "Chapter 7 PDF visual identity")
    assert_path("Chapter 7 PDF visual identity", pdf_visual.get("path"), pdf_relative)
    assert_identity("Chapter 7 PDF visual identity", pdf_visual, pdf_row)
    if pdf_qa.get("sha256") != pdf_row["sha256"]:
        raise RuntimeError("Chapter 7 PDF visual-QA top-level hash is stale")
    if not isinstance(pdf_qa.get("pages"), int) or int(pdf_qa["pages"]) <= 0:
        raise RuntimeError("Chapter 7 PDF visual QA has no valid page count")
    ...
    if len(all_source_names) != EXPECTED_TOTAL_SOURCE_FILES or len(set(all_source_names)) != EXPECTED_TOTAL_SOURCE_FILES:
        raise RuntimeError("cumulative Chapters 1-7 source closure is not exactly 43 unique files")
    ...
    chapter_07 = {
        ...
        "files": chapter_files,
        "source_qa": "repo/qa/CHAPTER07_SOURCE_QA.json",
        "activities_and_explorations": 4,
        "exercises": 11,
        "tasks": 34,
        "assessable_prompts": 40,
    ...
    qa_receipts = {
        ...
        "backend": backend_row,
        "html_manifest": file_row("qa/CHAPTER07_HTML_MANIFEST.json"),
        "html_run_1": file_row("qa/CHAPTER07_HTML_MANIFEST_RUN1.json"),
        "html_run_2": file_row("qa/CHAPTER07_HTML_MANIFEST_RUN2.json"),
        "html": file_row("qa/CHAPTER07_HTML_QA.json"),
        "browser": file_row("qa/CHAPTER07_BROWSER_QA.json"),
        "pdf_run_1": file_row("qa/CHAPTER07_PDF_RUN1_HASH.json"),
        "pdf_run_2": file_row("qa/CHAPTER07_PDF_RUN2_HASH.json"),
        "pdf": file_row("qa/CHAPTER07_PDF_VISUAL_QA.json"),
    }
    ...
    caveats.append(
        "Whole-book figure provenance remains a final-release gate; both active Chapter 7 figures have Indonesian descriptions."
    )
    ...
    manifest = {
        ...
        "lane": "O003/C90",
        "locale": "id-ID",
        "boundary": "chapters_01_07_with_separately_licensed_self_study_companions",
        "status": "pass",
        "prior_admission": {**file_row(PRIOR_MANIFEST), "published_identity_pinned": True},
        "authority": {
            ...
            "chapter_07_ordered_sha256": authority_combined,
        },
        ...
        "chapter_07_identity_bundle": {
            ...
            "cumulative_reader_wrapper": file_row("source/chapters_01_07_reader.ptx"),
        ...
        "reader_artifacts": {
            "html": {
                "path": "repo/output/chapters01-07-html",
        ...
        json.dumps(
            {
                ...
                "chapter_07_combined_sha256": chapter_combined,
                "cumulative_combined_sha256": cumulative_combined,
