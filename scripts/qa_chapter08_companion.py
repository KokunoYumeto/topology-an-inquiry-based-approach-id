#!/usr/bin/env python3
"""Fail-closed coverage, rights, and source-link audit for Chapter 8."""
...
SOURCE_FILES = (
    "chap_open_sets.ptx",
    "sec_open_sets_intro.ptx",
    "sec_open_sets.ptx",
    "sec_union_int_open_sets.ptx",
    "sec_cont_open_sets.ptx",
    "sec_interior_set.ptx",
    "sec_open_sets_summ.ptx",
    "sec_open_sets_exer.ptx",
)
FILE_GROUPS = {
    "sec_open_sets_intro.ptx": (("intro", 6),),
    "sec_open_sets.ptx": (("open-set", 5),),
    "sec_union_int_open_sets.ptx": (("union-intersection", 6),),
    "sec_cont_open_sets.ptx": (("continuity-open-set", 7),),
    "sec_interior_set.ptx": (("interior", 6),),
    "sec_open_sets_exer.ptx": (("exercise", 25),),
}
FRAGMENTS = (
    "chapter_08_source_guides.ptx",
    "chapter_08_exercise_guides_a.ptx",
    "chapter_08_exercise_guides_b.ptx",
    "chapter_08_mastery.ptx",
)
EXPECTED_AUTHORITY_ORDERED_SHA256 = (
    "f26cd93560b36e2db4c7dc040e8701146b1f358471e419ab572da79015541ff4"
)
EXPECTED_AUTHORITY_RAW_SHA256 = (
    "22ea6d5e193fd6b209ecfb7fe36233a3fa3a94e8630dc907f390efd137faef3f"
)
EXPECTED_TERM_IDS = {f"O003-T{number:03d}" for number in range(104, 113)}
EXPECTED_CORRECTION_IDS = {f"O003-C{number:03d}" for number in range(73, 82)}
EXPECTED_SOURCE_PROMPTS = 55
EXPECTED_ACTIVITY_PROMPTS = 30
EXPECTED_EXERCISE_PROMPTS = 25
EXPECTED_MASTERY = 6
EXPECTED_EXTERNAL_XREFS = {
    "ex_1_over_1_plus_t_metric",
    "ex_MS_Q_metric",
    "lem_functions_subsets",
}
...
def guide_id(sequence: int) -> str:
    return f"o003-c90-ch08-guide-{sequence:02d}"
...
def expected_entry_ids() -> list[str]:
    return expected_source_entry_ids() + [
        f"o003-c90-ch08-mastery-{ordinal:02d}"
        for ordinal in range(1, EXPECTED_MASTERY + 1)
    ]


def prompt_nodes(tree: etree._ElementTree, name: str) -> list[Prompt]:
    """Return every assessable Chapter 8 prompt in reader order."""
    root = tree.getroot()
    if name != "sec_open_sets_exer.ptx":
        prompts: list[Prompt] = []
            ...
            mappings.append(
                {
                    ...
                    "source_anchor_id": (
                        f"o003-gvsu-ch08-{Path(name).stem}-prompt-{sequence:02d}"
                    ),
                    "group": group,
...
def main() -> int:
    ...
    companion = repo / "companion/chapter_08_open_sets_self_study.ptx"
    report_path = repo / "qa/CHAPTER08_COMPANION_QA.json"
    ...
    if missing:
        raise SystemExit(f"missing required Chapter 8 inputs: {missing}")

    source_qa = json.loads(source_qa_path.read_text(encoding="utf-8"))
    if source_qa.get("status") != "pass":
        failures.append("Chapter 8 source QA is not passing")
    ...
    if set(actual_insertions) != set(EXPECTED_INSERTIONS):
        failures.append("Chapter 8 approved element-insertion closure changed")
    else:
    ...
    if source_qa.get("approved_element_block_moves") not in ([], None):
        failures.append("unexpected approved Chapter 8 element block move")
    if source_qa.get("approved_element_shell_moves") not in ([], None):
        failures.append("unexpected approved Chapter 8 element shell move")
    ...
    if math_changes != [
        ...
        failures.append("Chapter 8 protected-math change is not exactly A to S")
    if set(source_qa.get("approved_external_xref_targets", [])) != EXPECTED_EXTERNAL_XREFS:
        failures.append("Chapter 8 approved external-xref closure changed")
    ...
    if ordered_digest.hexdigest() != EXPECTED_AUTHORITY_ORDERED_SHA256:
        failures.append("frozen Chapter 8 authority name-delimited identity changed")
    if raw_digest.hexdigest() != EXPECTED_AUTHORITY_RAW_SHA256:
        failures.append("frozen Chapter 8 authority raw-concatenation identity changed")
    ...
    root = tree.getroot()
    if root.get(XML_ID) != "o003-c90-ch08-companion":
        failures.append("unexpected Chapter 8 companion root ID")
    if root.get(XML_LANG) != "id-ID":
        failures.append("Chapter 8 companion root is not explicitly id-ID")
    ...
    if {row.get("id") for row in terms} != EXPECTED_TERM_IDS:
        failures.append("Chapter 8 terminology controls are incomplete")
    if any(row.get("status") != "approved" or not row.get("id_ID") for row in terms):
        failures.append("Chapter 8 terminology controls are not fully approved and populated")
    ...
    if {row.get("id") for row in corrections} != EXPECTED_CORRECTION_IDS:
        failures.append("Chapter 8 correction controls O003-C073 through O003-C081 are incomplete")
    correction_statuses = {row.get("id"): row.get("status") for row in corrections}
    expected_statuses = {
        **{f"O003-C{number:03d}": "verified" for number in (73, 74, 75, 76, 77, 80, 81)},
        "O003-C078": "unresolved",
        "O003-C079": "unresolved",
    }
    if correction_statuses != expected_statuses or any(not row.get("evidence") for row in corrections):
        failures.append("Chapter 8 correction statuses or evidence differ from the frozen ledger")
    ...
    report = {
        ...
        "companion": identity(companion, "companion/chapter_08_open_sets_self_study.ptx"),
        "fragments": [
        ...
        "entries": entries,
        "source_qa": identity(source_qa_path, "qa/CHAPTER08_SOURCE_QA.json"),
        "control_inputs": {
            "terminology": {
    ...
    print(
        json.dumps(
            {
                "report": identity(report_path, "qa/CHAPTER08_COMPANION_QA.json"),
