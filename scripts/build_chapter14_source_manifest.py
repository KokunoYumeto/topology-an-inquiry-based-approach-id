#!/usr/bin/env python3
"""Build the fail-closed cumulative Chapters 1-14 admission manifest.

Run only after the Chapter 14 source, final companion/backend, cumulative
...
OUTPUT = ROOT / "qa/CHAPTER14_SOURCE_MANIFEST.json"
MODEL = "OpenAI Codex gpt-5.6-sol, Ultra"

PRIOR_MANIFEST = "qa/CHAPTER13_SOURCE_MANIFEST.json"
PRIOR_MANIFEST_IDENTITY = {
    "bytes": 146_518,
    "sha256": "c485aaeb47a5ade57f1cd0c16ec1e65520f30795bfdf0a18f9138f5369f40ad4",
}
BOUNDARY = "chapters_01_14_with_separately_licensed_self_study_companions"
PRIOR_BOUNDARY = "chapters_01_13_with_separately_licensed_self_study_companions"
...
COMPANION_FILES = (
    "companion/chapter_14_source_guides_a.ptx",
    "companion/chapter_14_source_guides_b.ptx",
    "companion/chapter_14_source_guides_c.ptx",
    "companion/chapter_14_exercise_guides_a.ptx",
    "companion/chapter_14_exercise_guides_b.ptx",
    "companion/chapter_14_exercise_guides_c.ptx",
    "companion/chapter_14_exercise_guides_d.ptx",
    "companion/chapter_14_exercise_guides_e.ptx",
    "companion/chapter_14_exercise_guides_f.ptx",
    "companion/chapter_14_mastery.ptx",
)
COMPANION_WRAPPER = "companion/chapter_14_continuity_homeomorphisms_self_study.ptx"
READER_WRAPPER = "source/chapters_01_14_reader.ptx"
...
FINAL_BACKEND = "backend/chapter_14_companion_manifest.json"
FINAL_ALIASES = "backend/chapter_14_entry_aliases.csv"
PROMPT_MAP = "backend/chapter_14_source_prompt_map.csv"
GROUPING_BACKEND = "backend/chapter_14_grouping_nodes.json"
PROMPT_INVENTORY = "backend/chapter_14_prompt_inventory.json"

SOURCE_QA = "qa/CHAPTER14_SOURCE_COMPLETE_QA.json"
COMPANION_QA = "qa/CHAPTER14_COMPANION_QA.json"
COMPANION_WRAPPER_SCHEMA_QA = "qa/CHAPTER14_COMPANION_WRAPPER_SCHEMA_QA.json"
MASTERY_SCHEMA_QA = "qa/CHAPTER14_MASTERY_SCHEMA_QA.json"
SCHEMA_QA = "qa/CHAPTER14_CUMULATIVE_SCHEMA_QA.json"
HTML_MANIFEST = "qa/CHAPTER14_HTML_MANIFEST.json"
HTML_RUN_1 = "qa/CHAPTER14_HTML_MANIFEST_RUN1.json"
HTML_RUN_2 = "qa/CHAPTER14_HTML_MANIFEST_RUN2.json"
HTML_QA = "qa/CHAPTER14_HTML_QA.json"
BROWSER_QA = "qa/CHAPTER14_BROWSER_QA.json"
ASSET_PDF_QA = "qa/CHAPTER14_ASSET_PDF_METADATA_QA.json"
PDF_RECEIPTS = {
    "deterministic_run_1": "qa/CHAPTER14_PDF_RUN1_HASH.json",
    "deterministic_run_2": "qa/CHAPTER14_PDF_RUN2_HASH.json",
    "structure_qa": "qa/CHAPTER14_PDF_STRUCTURE.json",
    "visual_qa": "qa/CHAPTER14_PDF_VISUAL_QA.json",
}
DOCS_RECEIPTS = (
    "qa/CHAPTER14_DOCS_MANIFEST.json",
    "qa/CHAPTER14_DOCS_QA.json",
    "qa/CHAPTER14_BUILD_QA.md",
)
HTML_ROOT = "output/chapters01-14-html"
PDF_PATH = "output/chapters01-14-pdf/chapters_01_14_reader.pdf"
...
def validate_html(manifest: dict[str, Any]) -> tuple[int, int, int]:
    ...
    if set(actual) != set(expected):
        raise RuntimeError("cumulative Chapter 14 HTML closure differs from its manifest")
    total = 0
    html_files = 0
    ...
    if facts != (manifest.get("file_count"), manifest.get("html_files"), manifest.get("total_bytes")):
        raise RuntimeError("cumulative Chapter 14 HTML census differs")
    return facts
...
def validate_final_companion() -> tuple[dict[str, Any], dict[str, Any], list[dict[str, object]]]:
    ...
    if backend.get("partial") is not False or backend.get("status") != expected_status:
        raise RuntimeError("Chapter 14 backend is partial or has not reached final companion status")
    if companion_qa.get("status") != "pass" or companion_qa.get("failures") != []:
        raise RuntimeError("Chapter 14 companion QA is not passing")
    ...
    expected_correction_ids = [f"O003-C{number:03d}" for number in range(162, 182)]
    if corrections.get("ids") != expected_correction_ids:
        raise RuntimeError("Chapter 14 correction-ID closure changed")
    coverage = require_dict(backend.get("coverage"), "backend coverage")
    expected_coverage = {
        "covered_source_prompt_guides": 81,
        "pending_source_prompt_guides": 0,
        "covered_mastery_checks": 8,
        "entry_total": 89,
        "staged_surface_total": 356,
        "grouping_nodes": 3,
    }
    for key, expected in expected_coverage.items():
        if coverage.get(key) != expected:
            raise RuntimeError(f"Chapter 14 backend coverage changed: {key}")
    ...
    if actual_wrapper_includes != expected_wrapper_includes:
        raise RuntimeError(
            "Chapter 14 wrapper XInclude order or exact closure changed: "
...
def validate_project_targets() -> None:
    ...
    targets = {
        ...
        if element.get("name") in {"chapters01-14-html", "chapters01-14-pdf"}
    }
    if set(targets) != {"chapters01-14-html", "chapters01-14-pdf"}:
        raise RuntimeError("project.ptx lacks one exact cumulative Chapter 14 target")
    ...
    expected = {
        ...
        "chapters01-14-pdf": {
            ...
            "source": READER_WRAPPER,
            "publication": "publication/publication.ptx",
            "output-dir": "output/chapters01-14-pdf",
    ...
    if targets["chapters01-14-pdf"].get("pdf-method") != "xelatex":
        raise RuntimeError("Chapter 14 PDF target lost the pinned xelatex method")
    html_params = [
        (element.get("key"), element.get("value"))
        for element in targets["chapters01-14-html"].findall("stringparam")
    ]
    if html_params != [("html.css.extra", "external/o003-readable-layout.css")]:
        raise RuntimeError("Chapter 14 HTML target lost its exact readable-layout CSS")
...
def validate_html_determinism(final_manifest: dict[str, Any]) -> None:
    ...
    for key in ("file_count", "html_files", "total_bytes", "canonical_manifest_sha256"):
        ...
        if values[0] != values[1] or values[1] != values[2]:
            raise RuntimeError(f"Chapter 14 HTML double-build differs for {key}: {values!r}")
...
def validate_pdf_determinism(pdf_row: dict[str, object]) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    require_pass(run_1, "Chapter 14 PDF run 1")
    require_pass(run_2, "Chapter 14 PDF run 2")

    run_1_path = safe_relative(run_1.get("path"))
    if run_1_path != "tmp/pdfs/chapter14-run1.pdf":
        raise RuntimeError("Chapter 14 PDF run-1 snapshot path changed")
    snapshot_row = repo_row(run_1_path)
    assert_path_identity("Chapter 14 PDF run-1 snapshot", run_1, snapshot_row)
    assert_path_identity("Chapter 14 PDF run-2 artifact", run_2, pdf_row)
    for key in ("bytes", "sha256", "pages", "source_date_epoch"):
        if run_1.get(key) != run_2.get(key):
            raise RuntimeError(f"Chapter 14 PDF double-build differs for {key}")
    if run_2.get("byte_identical_to_run_1") is not True:
        raise RuntimeError("Chapter 14 PDF run 2 does not attest byte identity")
    assert_path_identity(
        "Chapter 14 PDF run-2 nested run-1 identity",
        run_2.get("run_1"),
        snapshot_row,
    )
    for run, expected_path in (
        (run_1, "qa/CHAPTER14_PDF_BUILD_RUN1.log"),
        (run_2, "qa/CHAPTER14_PDF_BUILD_RUN2.log"),
    ):
        transcript = require_dict(run.get("transcript"), "PDF run transcript")
        assert_path_identity("Chapter 14 PDF transcript", transcript, repo_row(expected_path))
    return run_1, run_2


def build_manifest() -> dict[str, Any]:
    assert_identity("pinned Chapter 13 manifest", PRIOR_MANIFEST_IDENTITY, identity(ROOT / PRIOR_MANIFEST))
    prior = read_json(PRIOR_MANIFEST)
    require_pass(prior, "Chapter 13 source manifest")
    if prior.get("boundary") != PRIOR_BOUNDARY or prior.get("pending_evidence") != []:
        raise RuntimeError("unexpected Chapter 13 admission boundary")
    ...
    if len(prior_names) != 90 or len(set(prior_names)) != 90:
        raise RuntimeError("prior translated source closure is not exactly 90 files")

    source_qa = read_json(SOURCE_QA)
    require_pass(source_qa, "Chapter 14 source QA")
    if source_qa.get("failures") != []:
        raise RuntimeError("Chapter 14 source QA records failures")
    qa_sources: dict[str, dict[str, Any]] = {}
    for raw in require_list(source_qa.get("files"), "Chapter 14 source QA files"):
        row = require_dict(raw, "Chapter 14 source QA row")
        name = row.get("file")
        if not isinstance(name, str) or not name or "/" in name or "\\" in name:
            raise RuntimeError(f"invalid Chapter 14 source-QA filename: {name!r}")
        if name in qa_sources:
            raise RuntimeError(f"duplicate Chapter 14 source-QA filename: {name}")
        qa_sources[name] = row
    if set(qa_sources) != set(CHAPTER_SOURCE_FILES):
        raise RuntimeError("Chapter 14 source QA closure is not the exact eight-file target")
    ...
    if source_qa.get("combined_translated_sha256") != chapter_combined:
        raise RuntimeError("Chapter 14 translated combined identity is stale")
    ...
    if MODEL not in reader_text:
        raise RuntimeError("Chapter 14 reader wrapper lacks exact model provenance")
    ...
    for key, value in zip(("file_count", "html_files", "total_bytes"), html_facts):
        if html_qa.get(key) != value:
            raise RuntimeError(f"Chapter 14 HTML QA differs for {key}")
    if html_qa.get("canonical_manifest_sha256") != html_manifest.get("canonical_manifest_sha256"):
        raise RuntimeError("Chapter 14 HTML canonical identity differs")

    pdf_visual = read_json(PDF_RECEIPTS["visual_qa"])
    require_pass(pdf_visual, "Chapter 14 PDF visual QA")
    pdf_row = repo_row(PDF_PATH)
    assert_path_identity("Chapter 14 PDF", pdf_visual.get("pdf"), pdf_row)
    ...
    if not isinstance(pdf_pages, int) or pdf_pages <= 0:
        raise RuntimeError("Chapter 14 PDF visual QA has no positive page count")
    if pdf_pages != pdf_run_2.get("pages"):
        raise RuntimeError("Chapter 14 PDF visual and deterministic page counts differ")
    assert_path_identity(
        "Chapter 14 PDF visual build log",
        pdf_visual.get("build_log"),
        repo_row("qa/CHAPTER14_PDF_BUILD_RUN2.log"),
    )
    structure_qa = read_json(PDF_RECEIPTS["structure_qa"])
    require_pass(structure_qa, "Chapter 14 PDF structure QA")
    assert_path_identity("Chapter 14 PDF structure artifact", structure_qa.get("artifact"), pdf_row)
    if structure_qa.get("pages") != pdf_pages:
        raise RuntimeError("Chapter 14 PDF structure page count differs")
    qa_paths = [
    ...
    translated["chapter_14"] = chapter_14
    ...
    manifest = {
        ...
        "cumulative_component_identities": {
            "translated_source_chapters_01_14": {
                "file_count": 98,
                "chapters": [f"chapter_{number:02d}" for number in range(1, 15)],
                "cumulative_combined_sha256": translated["cumulative_combined_sha256"],
            },
            "companions_chapters_01_14": cumulative_companions,
            "backends_chapters_01_14": cumulative_backends,
        },
        "companion_coverage": {
            "source_prompt_guides": 81,
            "mastery_checks": 8,
            "total_entries": 89,
        ...
        "known_caveats": [
            "This is a complete admitted 14-of-20 cumulative reader boundary, not the complete edition.",
    ...
    if (
        ...
        raise RuntimeError("Chapter 14 checkpoint scope is not represented truthfully")
    return manifest
...
def main() -> int:
    ...
    try:
        ...
        raise SystemExit(f"Chapter 14 admission manifest gate failed: {exc}") from exc
    summary = {
    ...
    if not args.check_only:
        ...
        summary.update({"output": "repo/qa/CHAPTER14_SOURCE_MANIFEST.json", **identity(OUTPUT)})
