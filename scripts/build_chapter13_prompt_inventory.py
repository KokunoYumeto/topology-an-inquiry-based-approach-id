#!/usr/bin/env python3
"""Build the deterministic Chapter 13 prompt inventory and bootstrap backend.
...
the Chapter 13 inventory, prompt map, and grouping-node backend.
"""
...
CHAPTER_FILE = "chap_Closed_sets_topology.ptx"
...
FROZEN_ORDERED_SHA256 = (
    "d39d380b09e0cd121b0e134745da16a519507c32bbf8ff782ea38ced163dc5db"
)
AUTHORITY_SUBTREE_HASH_CONTRACT = "sha256-c14n-1.0-with-comments"
SOURCE_ANCHOR_KIND = "synthetic_locale_neutral_alias"

EXPECTED_PROMPT_TOTAL = 76
EXPECTED_NONEXERCISE_TOTAL = 30
EXPECTED_EXERCISE_TOTAL = 46
EXPECTED_GROUPING_TOTAL = 7
EXERCISE_BATCHES = (
    ("a", 10),
    ("b", 10),
    ("c", 5),
...
INVENTORY_PATH = BACKEND / "chapter_13_prompt_inventory.json"
PROMPT_MAP_PATH = BACKEND / "chapter_13_source_prompt_map.csv"
GROUPING_PATH = BACKEND / "chapter_13_grouping_nodes.json"
...
def load_authority_closure() -> tuple[list[dict[str, Any]], dict[str, etree._ElementTree]]:
    ...
    if not chapter_path.is_file():
        raise SystemExit(f"missing frozen Chapter 13 wrapper: {chapter_path}")
    ...
    if hrefs != EXPECTED_CHAPTER_INCLUDES:
        raise SystemExit(
            "frozen Chapter 13 XInclude order changed: "
    ...
    if len(names) != EXPECTED_AUTHORITY_FILE_COUNT or len(set(names)) != len(names):
        raise SystemExit("Chapter 13 authority closure is not exactly nine unique files")

    records: list[dict[str, Any]] = []
    documents: dict[str, etree._ElementTree] = {}
    for file_sequence, name in enumerate(names, start=1):
        if Path(name).name != name:
            raise SystemExit(f"nonlocal Chapter 13 include is forbidden: {name}")
        path = SOURCE_DIR / name
        if not path.is_file():
            raise SystemExit(f"missing frozen Chapter 13 authority file: {path}")
    ...
    if actual_ordered_hash != FROZEN_ORDERED_SHA256:
        raise SystemExit(
            "frozen Chapter 13 ordered authority hash changed: "
...
def exercise_entry_id(ordinal: int) -> tuple[str, str, int]:
    ...
    for letter, count in EXERCISE_BATCHES:
        if cursor <= count:
            return f"o003-c90-ch13-exer-{letter}-{cursor:02d}", letter, cursor
        cursor -= count
...
def build_inventory() -> tuple[bytes, bytes, bytes, dict[str, Any]]:
    ...
    for event in group_events:
        ...
        source_anchor = (
            f"o003-gvsu-ch13-{context}-group-{alias_counters[counter_key]:02d}"
        )
    ...
    for sequence, event in enumerate(prompt_events, start=1):
        ...
        if partition == "exercise":
            ...
            entry_id = f"o003-c90-ch13-guide-{nonexercise_ordinal:02d}"
            batch = None
            batch_sequence = None
        ...
        source_anchor = (
            f"o003-gvsu-ch13-{context}-{label}-"
    ...
    if len(set(entry_ids)) != len(entry_ids):
        raise SystemExit("generated Chapter 13 entry IDs are not unique")
    if len(set(source_anchors)) != len(source_anchors):
        raise SystemExit("generated Chapter 13 source aliases are not unique")
    if [entry["sequence"] for entry in entries] != list(range(1, 77)):
        raise SystemExit("generated Chapter 13 entries are not in exact order 1..76")
    ...
    grouping_payload = {
        "schema_version": 1,
        "unit": "o003-c90-ch13-closed-sets-topological-spaces",
        "locator_contract": {
            "authority_commit": FROZEN_AUTHORITY_COMMIT,
    ...
    inventory_payload = {
        ...
        "unit": {
            ...
            "sequence": 13,
            "source_chapter_xml_id": "chap_Closed_sets_topology",
        },
        "authority": {
        ...
        "source_anchor_contract": {
            ...
            "format": (
                "o003-gvsu-ch13-{nearest-real-xml-id}-{task|statement|body|group}-NN"
            ),
        ...
        "entry_id_contract": {
            ...
            "exercise": {
                "format": "o003-c90-ch13-exer-{batch}-{NN}",
        ...
        "prompt_map": {
            "path": "backend/chapter_13_source_prompt_map.csv",
            **identity_bytes(prompt_payload),
            "fields": list(PROMPT_MAP_FIELDS),
            "row_count": len(prompt_rows),
        },
        "grouping_backend": {
            "path": "backend/chapter_13_grouping_nodes.json",
