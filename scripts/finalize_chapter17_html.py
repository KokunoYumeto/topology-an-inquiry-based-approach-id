#!/usr/bin/env python3
"""Deterministically localize the PreTeXt 1.7.5 Chapters 1-17 HTML boundary."""
...
def transform(path: Path, root_directory: Path, check_only: bool) -> tuple[int, list[str]]:
    ...
    # and GitHub Pages. Bind the original, local SVG icon explicitly. This is
    # deterministic and idempotent across cumulative reader boundaries.
...
def main() -> int:
    ...
    if residue:
        print(
            json.dumps(
                {"forbidden_visible_residue": residue},
                ensure_ascii=False,
                indent=2,
            )
        )
