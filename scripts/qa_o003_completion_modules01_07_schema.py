#!/usr/bin/env python3
"""Generate the pinned cumulative schema and ID/xref receipt for completion Modules 1–7."""
...
READER = ROOT / "source" / "o003_completion_modules_01_07_reader.ptx"
WRAPPER = ROOT / "completion" / "o003_c90_completion_self_study.ptx"
OUTPUT = ROOT / "qa" / "O003_COMPLETION_MODULES01_07_SCHEMA_QA.json"
...
EXPECTED_READER_HREFS = tuple(f"../completion/module_{number:02d}_{name}.ptx" for number, name in (
...
def expanded_validation(path: Path, relaxng: etree.RelaxNG) -> tuple[list[etree._Element], list[str]]:
    tree = parse(path)
    tree.xinclude()
    valid = relaxng.validate(tree)
    diagnostics = [str(entry) for entry in relaxng.error_log]
    if not valid or diagnostics:
        raise SystemExit(f"schema validation failed for {path.relative_to(ROOT)}: {diagnostics}")
    return [node for node in tree.getroot().iter() if isinstance(node.tag, str)], diagnostics
...
def build() -> bytes:
    ...
    if hrefs != EXPECTED_READER_HREFS:
        raise SystemExit(f"cumulative include closure changed: {hrefs}")
    wrapper_hrefs = tuple(parse(WRAPPER).xpath("//xi:include/@href", namespaces={"xi": XI_NS}))
    expected_wrapper_hrefs = tuple(value.replace("../completion/", "./") for value in EXPECTED_READER_HREFS)
    ...
    nodes, diagnostics = expanded_validation(READER, relaxng)
    ...
    report = {
        ...
        "xinclude": {"all_local": True, "closure_file_count": 8, "closure": [READER.relative_to(ROOT).as_posix()] + [path.relative_to(ROOT).as_posix() for path in modules]},
        ...
        "counts": {"modules": 7, "expanded_elements": len(nodes), "xml_ids": len(ids), "unique_xml_ids": len(set(ids)), "xrefs": len(refs), "unique_xref_targets": len(set(refs))},
    }
