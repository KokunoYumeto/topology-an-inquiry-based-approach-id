#!/usr/bin/env python3
"""Generate and fail-closed validate the Chapter 10 modular backend."""
...
SOURCE_ROLES = {
    "chap_closed_sets.ptx": "chapter_container",
    "sec_closed_sets_intro.ptx": "boundary_limit_and_isolated_points",
    "sec_closed_sets_metric.ptx": "closed_sets_in_metric_spaces",
    "sec_cont_closed_sets.ptx": "continuity_via_closed_sets",
    "sec_limit_bound_iso_seq.ptx": "sequential_boundary_limit_and_isolated_points",
    "sec_limit_closed.ptx": "closed_sets_and_limit_points",
    "sec_set_closure.ptx": "boundary_and_closure",
    "sec_closed_limits_seq.ptx": "sequential_characterization_of_closed_sets",
    "sec_closed_set_summ.ptx": "chapter_summary",
    "sec_closed_set_exer.ptx": "chapter_exercises",
}
ALIAS_FIELDS = (
    "companion_entry_id",
    "entry_kind",
...
GROUP_CONCEPTS = {
    "sec_closed_sets_intro": ("boundary_point", "limit_point", "isolated_point"),
    "sec_closed_sets_metric": ("closed_set", "open_complement", "metric_space"),
    "sec_cont_closed_sets": ("continuity", "closed_preimage", "closed_set_characterization"),
    "sec_limit_bound_iso_seq": ("sequence", "boundary_point", "limit_point", "isolated_point"),
    "sec_limit_closed": ("closed_set", "limit_point", "derived_set"),
    "sec_set_closure": ("closure", "boundary", "interior", "closed_set"),
    "sec_closed_limits_seq": ("sequentially_closed", "sequence_limit", "closed_set"),
    "sec_closed_set_exer": ("closed_sets_exercise_synthesis",),
    "mastery": ("closed_sets_transfer", "closed_sets_mastery"),
}
...
def mastery_entry_ids() -> list[str]:
    return [f"o003-c90-ch10-mastery-{ordinal:02d}" for ordinal in range(1, EXPECTED_MASTERY + 1)]
...
def concepts_for(group: str, ordinal: int) -> list[str]:
    ...
    return ["metric_space", "closed_sets", *concepts, f"{group}_prompt_{ordinal:02d}"]
...
def main() -> int:
    ...
    companion_path = repo / "companion/chapter_10_closed_sets_self_study.ptx"
    aliases_path = repo / "backend/chapter_10_entry_aliases.csv"
    output_path = repo / "backend/chapter_10_companion_manifest.json"
    terminology_path = lane / "00_control/TERMINOLOGY.csv"
    corrections_path = lane / "00_control/SOURCE_CORRECTIONS.csv"
    source_qa_path = repo / "qa/CHAPTER10_SOURCE_QA.json"
    companion_qa_path = repo / "qa/CHAPTER10_COMPANION_QA.json"
    ...
    if missing:
        raise SystemExit(f"missing Chapter 10 backend inputs: {missing}")
    ...
    if source_qa.get("status") != "pass":
        raise SystemExit("Chapter 10 source QA is not passing")
    if companion_qa.get("status") != "pass":
        raise SystemExit("Chapter 10 companion QA is not passing")
    if companion_qa.get("companion", {}).get("sha256") != file_identity(companion_path)["sha256"]:
        raise SystemExit("Chapter 10 companion QA is stale")
    fragment_hashes = {
    ...
    for name in FRAGMENTS:
        if fragment_hashes.get(name) != file_identity(repo / "companion" / name)["sha256"]:
            raise SystemExit(f"Chapter 10 companion QA fragment identity is stale: {name}")
    ...
    if ordered_digest.hexdigest() != EXPECTED_AUTHORITY_ORDERED_SHA256:
        raise SystemExit("frozen Chapter 10 authority name-delimited identity changed")
    if raw_digest.hexdigest() != EXPECTED_AUTHORITY_RAW_SHA256:
        raise SystemExit("frozen Chapter 10 authority raw-concatenation identity changed")

    companion_tree = etree.parse(str(companion_path), parser)
    companion_tree.xinclude()
    companion_root = companion_tree.getroot()
    if companion_root.get(XML_ID) != "o003-c90-ch10-companion":
        raise SystemExit("unexpected Chapter 10 companion root ID")
    ...
    for order, name in enumerate(SOURCE_FILES):
        ...
        for node in translated_tree.getroot().iter():
            ...
            structural_nodes.append({
                "id": xml_id or f"o003-gvsu-ch10-{Path(name).stem}-{tag}-{ordinal_by_tag[tag]:02d}",
    ...
    if mapping_failures:
        raise SystemExit(f"Chapter 10 prompt mapping failed: {mapping_failures}")
    if len(qa_mappings) != EXPECTED_SOURCE_PROMPTS:
        raise SystemExit("expected exactly 80 ordered Chapter 10 source prompt mappings")
    source_ids = expected_source_entry_ids()
    if [row["companion_entry_id"] for row in qa_mappings] != source_ids:
        raise SystemExit("Chapter 10 source prompt mapping IDs changed")
    if companion_qa.get("source_prompt_mappings") != qa_mappings:
        raise SystemExit("Chapter 10 companion QA prompt mappings are stale")
    ...
    for ordinal, mastery_id in enumerate(mastery_entry_ids(), start=1):
        alias_rows.append({
            ...
            "source_anchor_id": f"o003-c90-ch10-original-mastery-{ordinal:02d}",
            "anchor_origin": "original",
            "source_group": "mastery",
            "ordinal_within_group": ordinal,
            "prompt_kind": "original_mastery_exercise",
            "source_context_id": "o003-c90-ch10-mastery",
            "authority_file": "",
            "authority_line": "",
            "authority_selector": "",
            "authority_statement_sha256": "",
    ...
    if dict(counts) != {"source_prompt_guide": 80, "mastery_check": 8}:
        raise SystemExit(f"entry coverage mismatch: {dict(counts)}")
    expected_surfaces = {kind: 88 for kind in ("statement", "hint", "answer", "solution")}
    ...
    if {row.get("id") for row in terms} != EXPECTED_TERM_IDS:
        raise SystemExit("Chapter 10 terminology controls are incomplete")
    with corrections_path.open(encoding="utf-8-sig", newline="") as handle:
        corrections = sorted(
            (row for row in csv.DictReader(handle) if row.get("unit") == "chapter_10_closed_sets_metric"),
            key=lambda row: str(row.get("id")),
        )
    if {row.get("id") for row in corrections} != EXPECTED_CORRECTION_IDS:
        raise SystemExit("Chapter 10 source-correction controls are incomplete")
    ...
    manifest = {
        ...
        "locale": "id-ID",
        "stable_id_namespace": {
            "prefix": "o003-c90-ch10",
            "source_anchor_prefix": "o003-gvsu-ch10",
            "policy": "locale-neutral deterministic IDs; upstream XML IDs retained where present",
        },
        "unit": {
            "id": "o003-c90-ch10-closed-sets-metric-spaces",
            "sequence": 10,
            "title": "Himpunan Tertutup di Ruang Metrik",
            "prerequisites": ["o003-c90-ch03-metric-spaces", "o003-c90-ch09-sequences-metric-spaces"],
            "concepts": [
                "boundary_point", "limit_point", "isolated_point", "closed_set",
                "closed_preimage", "closure", "boundary", "sequentially_closed",
            ],
        },
        "component": {
            "id": "o003-c90-ch10-companion",
            "type": "self_study_companion",
            "title": "Pendamping Mandiri Bab 10: Himpunan Tertutup dalam Ruang Metrik",
            "path": "repo/companion/chapter_10_closed_sets_self_study.ptx",
            "fragments": [
                {"path": f"repo/companion/{name}", **file_identity(repo / "companion" / name)} for name in FRAGMENTS
            ],
            "entry_alias_map": {"path": "repo/backend/chapter_10_entry_aliases.csv", **file_identity(aliases_path)},
        ...
        "translated_unit_source_qa": {
            "path": "repo/qa/CHAPTER10_SOURCE_QA.json",
        ...
        "companion_qa": {
            "path": "repo/qa/CHAPTER10_COMPANION_QA.json",
        ...
        "provenance": {
            ...
            "translation_source_qa": "repo/qa/CHAPTER10_SOURCE_QA.json",
            "companion_authorship": "original_id_ID_expression",
        },
        "coverage_contract": {
            "source_prompt_guides": 80,
            "activity_or_task_guides": 43,
            "exercise_prompt_guides": 37,
            "excluded_grouping_tasks": 5,
            "mastery_checks": 8,
            "total_entries": 88,
