#!/usr/bin/env python3
"""Assemble the full admitted Chapters 1-13 Zenodo package.

This builder consumes only the final cumulative Chapter 13 source manifest and
the admitted Chapter 13 HTML/PDF.  It never reads or advances the historical
Chapter 13 partial-package directory.  ``--check-only`` validates the frozen
inputs without creating package artifacts.
"""
...
BASE = "topologi-pendekatan-berbasis-inkuiri-bab-01-13"
...
PDF = ROOT / "output/chapters01-13-pdf/chapters_01_13_reader.pdf"
HTML_ROOT = ROOT / "output/chapters01-13-html"
SOURCE_MANIFEST = ROOT / "qa/CHAPTER13_SOURCE_MANIFEST.json"
HTML_MANIFEST = ROOT / "qa/CHAPTER13_HTML_MANIFEST.json"
AUTHORITY_ARCHIVE = LANE / "authority/archives/gvsuoer-topology-0c2d8f614ef87aa00de373f3418146c2f1d13bb9.zip"

REQUIRED_INVENTORY = {
    "repo/source/chapters_01_13_reader.ptx",
    "repo/companion/chapter_13_closed_sets_topological_spaces_self_study.ptx",
    "repo/companion/chapter_13_mastery.ptx",
    "repo/backend/chapter_13_companion_manifest.json",
    "repo/backend/chapter_13_entry_aliases.csv",
    "repo/backend/chapter_13_source_prompt_map.csv",
    "repo/backend/chapter_13_grouping_nodes.json",
    "repo/qa/CHAPTER13_SOURCE_QA.json",
    "repo/qa/CHAPTER13_COMPANION_QA.json",
    "repo/qa/CHAPTER13_CUMULATIVE_SCHEMA_QA.json",
    "repo/qa/CHAPTER13_HTML_MANIFEST.json",
    "repo/qa/CHAPTER13_HTML_QA.json",
    "repo/qa/CHAPTER13_BROWSER_QA.json",
    "repo/qa/CHAPTER13_ASSET_PDF_METADATA_QA.json",
    "repo/qa/CHAPTER13_PDF_VISUAL_QA.json",
    "repo/qa/CHAPTER13_BUILD_QA.md",
}
...
def html_entries(
    ...
    for name, path in (
        ...
        ("CHAPTER13_HTML_MANIFEST.json", HTML_MANIFEST),
    ):
...
def source_entries(source_manifest: dict[str, object]) -> dict[str, bytes]:
    ...
    if missing:
        raise RuntimeError(f"final source inventory omits required Chapter 13 files: {sorted(missing)}")
    ...
    manifest_name = f"{prefix}/repo/qa/CHAPTER13_SOURCE_MANIFEST.json"
...
def validate_inputs() -> tuple[dict[str, object], dict[str, object], dict[str, object], int]:
    ...
    if (
        ...
        raise RuntimeError("source manifest is not a truthful admitted Chapter 13 checkpoint")
    ...
    if pdf_row.get("status") != "pass" or pdf_row.get("path") != f"repo/{PDF.relative_to(ROOT).as_posix()}":
        raise RuntimeError("source manifest has not admitted the exact Chapter 13 PDF")
    expected_pdf = {
    ...
    require_identity(PDF, expected_pdf, "Chapter 13 PDF")
    pdf_reader = assert_pdf_privacy(PDF)
    if len(pdf_reader.pages) != pages:
        raise RuntimeError("Chapter 13 PDF page count changed")

    html_row = require_dict(reader.get("html"), "admitted HTML")
    if html_row.get("status") != "deterministic_tree_pass":
        raise RuntimeError("source manifest has not admitted deterministic Chapter 13 HTML")
    expected_html = {
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=ROOT / "output/zenodo-chapters01-13",
    ...
    parser.add_argument(
        ...
        type=int,
        help="Latest published Zenodo record in the existing concept lineage; required for assembly.",
    )
    parser.add_argument(
        "--concept-doi",
        default="10.5281/zenodo.22059894",
...
def main() -> int:
    ...
    try:
        ...
        # bytes are written and the historical partial package is never opened.
        ...
        raise SystemExit(f"Chapter 13 package admission gate failed: {exc}") from exc
    if args.check_only:
        print(json.dumps({
            "status": "pass",
            "partial": True,
        ...
        return 0
    if not args.version or args.predecessor_record_id is None:
        raise SystemExit("--version and --predecessor-record-id are required for package assembly")
    ...
    file_rows = [
        {"path": PDF_NAME, **identity(pdf_target), "role": f"{pages}-page Indonesian cumulative Chapters 1-13 reader PDF"},
        {**html_zip, "role": "admitted cumulative Chapters 1-13 HTML reader and rights notes"},
        {**source_zip, "role": "editable Chapters 1-13 source, companions, final backend, build code, authority identity, and sanitized QA"},
    ...
    package_manifest = {
        ...
        "record": {
            ...
            "language": "ind",
            "completion": {"chapters_verified": 13, "chapters_total": 20, "complete_edition": False, "admitted_boundary_complete": True},
        },
        "authority": {
        ...
        "reader": {
            ...
            "html": expected_html,
            "source_files_translated": 90,
            "source_manifest": {"path": "qa/CHAPTER13_SOURCE_MANIFEST.json", **identity(SOURCE_MANIFEST)},
        },
        "rights": [
        ...
        "caveats": [
            "This is a verified 13-of-20 admitted boundary, not the complete edition.",
