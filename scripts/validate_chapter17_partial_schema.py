#!/usr/bin/env python3
"""Validate Chapter 17 in the sealed cumulative Chapters 1--16 context.

The checker inserts the Chapter 17 XInclude in memory immediately after the
admitted Chapter 16 include, expands only the local closure, validates it
against the pinned PreTeXt RelaxNG schema, and writes a portable JSON receipt.
It does not create or imply admission of a cumulative Chapters 1--17 reader.
"""
...
def chapter_closure(entry: Path, repo: Path) -> list[Path]:
    ...
    while pending:
        ...
        if not current.is_file() or repo.resolve() not in (current, *current.parents):
            raise RuntimeError(f"invalid local Chapter 17 include: {current}")
        seen.add(current)
        ...
        for node in tree.iter(XI):
            ...
            if not href or "://" in href or href.startswith(("/", "\\")):
                raise RuntimeError(f"nonlocal Chapter 17 include in {current}: {href}")
...
def main() -> int:
    ...
    base = repo / "source" / "chapters_01_16_reader.ptx"
    chapter = repo / "source" / "chap_Compact_topology.ptx"
    ...
    namespace = {"xi": XI_NS}
    prior = tree.xpath(
        '//xi:include[@href="./chap_quotients.ptx"]', namespaces=namespace
    )
    if len(prior) != 1:
        raise RuntimeError("could not identify the unique admitted Chapter 16 include")
    if tree.xpath(
        '//xi:include[@href="./chap_Compact_topology.ptx"]', namespaces=namespace
    ):
        raise RuntimeError("base wrapper already includes Chapter 17")

    include = etree.Element(XI)
    include.set("href", "./chap_Compact_topology.ptx")
    ...
    report = {
        ...
        "validation_context": (
            "Chapter 17 inserted in memory immediately after Chapter 16 in "
            "the sealed cumulative Chapters 1-16 book wrapper"
        ),
        "base_wrapper": identity(base, base.relative_to(repo).as_posix()),
        "inserted_include": "./chap_Compact_topology.ptx",
        "chapter_closure": [
