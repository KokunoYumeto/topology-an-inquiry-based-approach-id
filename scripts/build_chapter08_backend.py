#!/usr/bin/env python3
"""Generate and fail-closed validate the Chapter 8 modular backend."""
...
SOURCE_ROLES = {
    "chap_open_sets.ptx": "chapter_container",
    "sec_open_sets_intro.ptx": "motivating_open_sets_and_interior",
    "sec_open_sets.ptx": "open_sets_in_metric_spaces",
    "sec_union_int_open_sets.ptx": "unions_and_intersections_of_open_sets",
    "sec_cont_open_sets.ptx": "continuity_via_inverse_images_of_open_sets",
    "sec_interior_set.ptx": "interior_of_a_set",
    "sec_open_sets_summ.ptx": "chapter_summary",
    "sec_open_sets_exer.ptx": "chapter_exercises",
}
ALIAS_FIELDS = (
    "companion_entry_id",
    "entry_kind",
...
CONCEPTS_BY_GROUP: dict[str, tuple[tuple[str, ...], ...]] = {
    "intro": (
        ("interior_point", "half_open_interval", "real_line"),
        ("interior_point", "subspace_metric", "metric_dependence"),
        ("interior_point", "discrete_metric", "finite_space"),
        ("interior", "intervals", "endpoint_analysis"),
        ("interior", "finite_subset", "empty_interior"),
        ("interior", "rational_numbers", "density"),
    ),
    "open-set": (
        ("empty_set", "open_set"),
        ("whole_space", "open_set"),
        ("union_of_open_balls", "open_set_proof_plan"),
        ("local_ball_containment", "union_membership"),
        ("union_of_open_balls", "open_set_characterization"),
    ),
    "union-intersection": (
        ("finite_union", "open_sets"),
        ("finite_intersection", "open_sets"),
        ("indexed_union", "nested_intervals"),
        ("arbitrary_union", "openness"),
        ("indexed_intersection", "nested_intervals"),
        ("infinite_intersection", "counterexample_to_openness"),
    ),
    "continuity-open-set": (
        ("continuity", "inverse_image_of_open_set", "proof_plan"),
        ("open_ball", "neighborhood", "codomain"),
        ("inverse_image", "neighborhood", "continuity"),
        ("inverse_image_of_open_set", "local_ball_containment"),
        ("biconditional", "continuity", "proof_directions"),
        ("inverse_image", "arbitrary_union", "set_identity"),
        ("inverse_image_of_open_set", "continuity_characterization"),
    ),
    "interior": (
        ("interior", "half_open_interval"),
        ("interior", "closed_interval"),
        ("interior", "isolated_points", "finite_union"),
        ("interior", "largest_open_subset", "proof_plan"),
        ("open_subset", "interior_membership", "local_ball"),
        ("interior", "largest_open_subset", "inclusion_proof"),
    ),
    "exercise": (
        ("discrete_metric", "every_subset_open"),
        ("discrete_domain", "every_function_continuous"),
        ("discrete_codomain", "counterexample", "continuity"),
        ("open_set", "fixed_point_of_interior"),
        ("interior", "monotonicity"),
        ("interior", "idempotence"),
        ("half_open_interval", "not_open"),
        ("metric_dependence", "euclidean_metric", "planar_set"),
        ("metric_dependence", "taxicab_metric", "planar_set"),
        ("metric_dependence", "maximum_metric", "planar_set"),
        ("finite_subset", "open_complement", "euclidean_plane"),
        ("continuity", "inverse_image_of_open_set", "biconditional"),
        ("rational_metric", "open_set_classification"),
        ("metric_separation", "disjoint_open_sets"),
        ("affine_function", "subspace_metric", "open_balls"),
        ("affine_function", "continuity", "domain_to_subspace"),
        ("affine_function", "continuity", "subspace_to_domain"),
        ("coordinate_projection", "continuity", "maximum_metric"),
        ("interior", "union", "false_statement"),
        ("interior", "union", "true_statement"),
        ("interior", "intersection", "true_statement"),
        ("interior", "intersection", "equality"),
        ("subset_of_open_set", "counterexample"),
        ("equivalent_metrics", "same_open_sets"),
        ("subspace_metric", "clopen_component"),
    ),
    "mastery": (
        ("open_set", "local_ball_criterion"),
        ("arbitrary_union", "finite_intersection", "infinite_intersection"),
        ("open_ball_basis", "local_ball_containment"),
        ("continuity", "inverse_image_of_open_set"),
        ("interior", "largest_open_subset", "idempotence"),
        ("open_closed_clopen", "metric_dependence", "counterexample"),
    ),
}
...
def mastery_entry_ids() -> list[str]:
    return [f"o003-c90-ch08-mastery-{ordinal:02d}" for ordinal in range(1, 7)]
...
def concepts_for(group: str, ordinal: int) -> list[str]:
    ...
    base = ["metric_space", "open_sets_in_metric_spaces"]
...
def main() -> int:
    ...
    companion_path = repo / "companion/chapter_08_open_sets_self_study.ptx"
    aliases_path = repo / "backend/chapter_08_entry_aliases.csv"
    output_path = repo / "backend/chapter_08_companion_manifest.json"
    terminology_path = lane / "00_control/TERMINOLOGY.csv"
    corrections_path = lane / "00_control/SOURCE_CORRECTIONS.csv"
    source_qa_path = repo / "qa/CHAPTER08_SOURCE_QA.json"
    companion_qa_path = repo / "qa/CHAPTER08_COMPANION_QA.json"
    ...
    if missing:
        raise SystemExit(f"missing Chapter 8 backend inputs: {missing}")
    ...
    if source_qa.get("status") != "pass":
        raise SystemExit("Chapter 8 source QA is not passing")
    if companion_qa.get("status") != "pass":
        raise SystemExit("Chapter 8 companion QA is not passing")
    if companion_qa.get("companion", {}).get("sha256") != file_identity(companion_path)["sha256"]:
        raise SystemExit("Chapter 8 companion QA is stale")
    fragment_hashes = {
    ...
    for name in FRAGMENTS:
        if fragment_hashes.get(name) != file_identity(repo / "companion" / name)["sha256"]:
            raise SystemExit(f"Chapter 8 companion QA fragment identity is stale: {name}")
    ...
    if ordered_digest.hexdigest() != EXPECTED_AUTHORITY_ORDERED_SHA256:
        raise SystemExit("frozen Chapter 8 authority closure identity changed")

    companion_tree = etree.parse(str(companion_path), parser)
    companion_tree.xinclude()
    companion_root = companion_tree.getroot()
    if companion_root.get(XML_ID) != "o003-c90-ch08-companion":
        raise SystemExit("unexpected Chapter 8 companion root ID")
    companion_elements = [
    ...
    for order, name in enumerate(SOURCE_FILES):
        ...
        for node in translated_tree.getroot().iter():
            ...
            structural_nodes.append(
                {
                    "id": xml_id
                    or f"o003-gvsu-ch08-{Path(name).stem}-{tag}-{ordinal_by_tag[tag]:02d}",
    ...
    if mapping_failures:
        raise SystemExit(f"Chapter 8 prompt mapping failed: {mapping_failures}")
    if len(qa_mappings) != EXPECTED_SOURCE_PROMPTS:
        raise SystemExit("expected exactly 55 ordered Chapter 8 source prompt mappings")
    source_ids = expected_source_entry_ids()
    if [row["companion_entry_id"] for row in qa_mappings] != source_ids:
        raise SystemExit("Chapter 8 source prompt mapping IDs changed")
    if companion_qa.get("source_prompt_mappings") != qa_mappings:
        raise SystemExit("Chapter 8 companion QA prompt mappings are stale")
        ...
        alias_rows.append(
            {
                ...
                "source_anchor_id": f"o003-c90-ch08-original-mastery-{ordinal:02d}",
                "anchor_origin": "original",
                "source_group": "mastery",
                "ordinal_within_group": ordinal,
                "prompt_kind": "original_mastery_exercise",
                "source_context_id": "o003-c90-ch08-mastery",
                "authority_file": "",
                "authority_line": "",
                "authority_selector": "",
                "authority_statement_sha256": "",
    ...
    if dict(counts) != {"source_prompt_guide": 55, "mastery_check": 6}:
        raise SystemExit(f"entry coverage mismatch: {dict(counts)}")
    expected_surfaces = {kind: 61 for kind in ("statement", "hint", "answer", "solution")}
    ...
    if {row.get("id") for row in terms} != EXPECTED_TERM_IDS:
        raise SystemExit("Chapter 8 terminology controls are incomplete")
    with corrections_path.open(encoding="utf-8-sig", newline="") as handle:
        corrections = sorted(
            (
                row for row in csv.DictReader(handle)
                if row.get("unit") == "chapter_08_open_sets"
            ),
            key=lambda row: str(row.get("id")),
        )
    if {row.get("id") for row in corrections} != EXPECTED_CORRECTION_IDS:
        raise SystemExit("Chapter 8 source-correction controls are incomplete")
    ...
    for stem, source_name in IMAGE_USES.items():
        ...
        image_assets.append(
            {
                "id": f"o003-gvsu-ch08-{stem.lower().replace('_', '-')}",
    ...
    manifest = {
        ...
        "locale": "id-ID",
        "stable_id_namespace": {
            "prefix": "o003-c90-ch08",
            "source_anchor_prefix": "o003-gvsu-ch08",
            "policy": "locale-neutral deterministic IDs; upstream XML IDs retained where present",
        },
        "unit": {
            "id": "o003-c90-ch08-open-sets-metric-spaces",
            "sequence": 8,
            "title": "Himpunan Terbuka dalam Ruang Metrik",
            "prerequisites": [
                "o003-c90-ch03-metric-spaces",
                "o003-c90-ch07-open-balls-neighborhoods-metric-spaces",
            ],
            "concepts": [
                "open_sets_in_metric_spaces",
                "open_ball_basis",
                "arbitrary_unions",
                "finite_intersections",
                "continuity_via_inverse_images",
                "interior",
                "metric_dependence",
                "clopen_sets",
            ],
        ...
        "component": {
            "id": "o003-c90-ch08-companion",
            "type": "self_study_companion",
            "title": "Pendamping Mandiri Bab 8: Himpunan Terbuka dalam Ruang Metrik",
            "path": "repo/companion/chapter_08_open_sets_self_study.ptx",
            "fragments": [
                {"path": f"repo/companion/{name}", **file_identity(repo / "companion" / name)}
                for name in FRAGMENTS
            ],
            "entry_alias_map": {
                "path": "repo/backend/chapter_08_entry_aliases.csv",
        ...
        "translated_unit_source_qa": {
            "path": "repo/qa/CHAPTER08_SOURCE_QA.json",
        ...
        "companion_qa": {
            "path": "repo/qa/CHAPTER08_COMPANION_QA.json",
