#!/usr/bin/env python3
"""Generate and fail-closed validate the Chapter 7 modular backend."""
...
SOURCE_ROLES = {
    "chap_open_balls.ptx": "chapter_container",
    "sec_open_balls_intro.ptx": "open_balls_in_metric_spaces",
    "sec_neighborhoods.ptx": "metric_neighborhoods",
    "sec_cont_neighborhoods.ptx": "continuity_via_neighborhoods",
    "sec_open_balls_summ.ptx": "chapter_summary",
    "sec_open_balls_exer.ptx": "chapter_exercises",
}
ALIAS_FIELDS = (
    "companion_entry_id",
    "entry_kind",
...
CONCEPTS_BY_GROUP: dict[str, tuple[tuple[str, ...], ...]] = {
    "intro": (
        ("open_ball", "euclidean_metric", "real_line"),
        ("open_ball", "euclidean_metric", "euclidean_plane"),
        ("open_ball", "maximum_metric", "euclidean_plane"),
        ("open_ball", "taxicab_metric", "euclidean_plane"),
        ("open_ball", "discrete_metric", "radius_threshold"),
    ),
    "neighborhood": (
        ("neighborhood", "open_ball", "triangle_inequality"),
        ("neighborhood", "nested_open_balls", "diagram_reasoning"),
        ("neighborhood", "converse_counterexample", "open_ball"),
    ),
    "continuity-ball": (
        ("inverse_image", "open_ball", "quadratic_function"),
        ("inverse_image", "open_ball", "quadratic_function"),
        ("inverse_image", "not_necessarily_open_ball", "counterexample"),
    ),
    "continuity-neighborhood": (
        ("epsilon_delta_continuity", "proof_planning"),
        ("open_ball", "neighborhood", "codomain"),
        ("inverse_image", "neighborhood_characterization"),
        ("domain_open_ball", "inverse_image_containment"),
        ("continuity_via_neighborhoods", "epsilon_delta_proof"),
    ),
    "exercise": (
        ("neighborhood_test", "euclidean_open_disk"),
        ("neighborhood_test", "taxicab_metric", "subspace_axis"),
        ("neighborhood_test", "rational_numbers", "density"),
        ("neighborhood_test", "rational_metric", "positive_integers"),
        ("finite_metric_space", "discrete_topology", "continuity"),
        ("hub_metric", "continuity", "isometry"),
        ("weighted_graph", "shortest_path_metric", "open_balls"),
        ("weighted_graph", "neighborhood_census"),
        ("linear_function", "inverse_image_containment", "continuity"),
        ("quadratic_function", "inverse_image_containment", "continuity"),
        ("distance_to_set", "lipschitz_continuity", "neighborhood"),
        ("distinct_points", "disjoint_neighborhoods", "metric_separation"),
        ("neighborhood_axioms", "existence"),
        ("neighborhood_axioms", "upward_closure"),
        ("neighborhood_axioms", "finite_intersection"),
        ("positivity_preservation", "continuity", "neighborhood"),
        ("discrete_metric", "every_subset_neighborhood"),
        ("true_false_counterexample", "open_ball", "neighborhood"),
        ("true_false_counterexample", "neighborhood", "point_membership"),
        ("true_false_counterexample", "continuous_image", "neighborhood"),
        ("inverse_image", "continuity_via_neighborhoods"),
        ("true_false_counterexample", "isolated_points", "open_ball"),
        ("finite_intersection", "neighborhood"),
        ("arbitrary_intersection", "neighborhood", "indexing_set"),
    ),
    "mastery": (
        ("open_ball", "comparison_of_metrics"),
        ("neighborhood", "largest_centered_radius"),
        ("continuity_via_neighborhoods", "inverse_image"),
        ("neighborhood_axioms", "finite_intersection"),
        ("counterexample", "arbitrary_intersection"),
        ("metric_separation", "disjoint_neighborhoods"),
    ),
}
...
def mastery_entry_ids() -> list[str]:
    return [f"o003-c90-ch07-mastery-{ordinal:02d}" for ordinal in range(1, 7)]
...
def concepts_for(group: str, ordinal: int) -> list[str]:
    ...
    base = ["metric_space", "open_balls_and_neighborhoods"]
...
def main() -> int:
    ...
    companion_path = repo / "companion/chapter_07_open_balls_self_study.ptx"
    aliases_path = repo / "backend/chapter_07_entry_aliases.csv"
    output_path = repo / "backend/chapter_07_companion_manifest.json"
    terminology_path = lane / "00_control/TERMINOLOGY.csv"
    corrections_path = lane / "00_control/SOURCE_CORRECTIONS.csv"
    source_qa_path = repo / "qa/CHAPTER07_SOURCE_QA.json"
    companion_qa_path = repo / "qa/CHAPTER07_COMPANION_QA.json"
    ...
    if missing:
        raise SystemExit(f"missing Chapter 7 backend inputs: {missing}")
    ...
    if source_qa.get("status") != "pass":
        raise SystemExit("Chapter 7 source QA is not passing")
    if companion_qa.get("status") != "pass":
        raise SystemExit("Chapter 7 companion QA is not passing")
    if companion_qa.get("companion", {}).get("sha256") != file_identity(companion_path)["sha256"]:
        raise SystemExit("Chapter 7 companion QA is stale")
    fragment_hashes = {
    ...
    for name in FRAGMENTS:
        if fragment_hashes.get(name) != file_identity(repo / "companion" / name)["sha256"]:
            raise SystemExit(f"Chapter 7 companion QA fragment identity is stale: {name}")
    ...
    if ordered_digest.hexdigest() != EXPECTED_AUTHORITY_ORDERED_SHA256:
        raise SystemExit("frozen Chapter 7 authority closure identity changed")

    companion_tree = etree.parse(str(companion_path), parser)
    companion_tree.xinclude()
    companion_root = companion_tree.getroot()
    if companion_root.get(XML_ID) != "o003-c90-ch07-companion":
        raise SystemExit("unexpected Chapter 7 companion root ID")
    companion_elements = [
    ...
    for order, name in enumerate(SOURCE_FILES):
        ...
        for node in translated_tree.getroot().iter():
            ...
            structural_nodes.append(
                {
                    "id": xml_id
                    or f"o003-gvsu-ch07-{Path(name).stem}-{tag}-{ordinal_by_tag[tag]:02d}",
    ...
    if mapping_failures:
        raise SystemExit(f"Chapter 7 prompt mapping failed: {mapping_failures}")
    if len(qa_mappings) != EXPECTED_SOURCE_PROMPTS:
        raise SystemExit("expected exactly 40 ordered Chapter 7 source prompt mappings")
    source_ids = expected_source_entry_ids()
    if [row["companion_entry_id"] for row in qa_mappings] != source_ids:
        raise SystemExit("Chapter 7 source prompt mapping IDs changed")
    if companion_qa.get("source_prompt_mappings") != qa_mappings:
        raise SystemExit("Chapter 7 companion QA prompt mappings are stale")
        ...
        alias_rows.append(
            {
                ...
                "source_anchor_id": f"o003-c90-ch07-original-mastery-{ordinal:02d}",
                "anchor_origin": "original",
                "source_group": "mastery",
                "ordinal_within_group": ordinal,
                "prompt_kind": "original_mastery_exercise",
                "source_context_id": "o003-c90-ch07-mastery",
                "authority_file": "",
                "authority_line": "",
                "authority_selector": "",
                "authority_statement_sha256": "",
    ...
    if dict(counts) != {"source_prompt_guide": 40, "mastery_check": 6}:
        raise SystemExit(f"entry coverage mismatch: {dict(counts)}")
    expected_surfaces = {kind: 46 for kind in ("statement", "hint", "answer", "solution")}
    ...
    if {row.get("id") for row in terms} != EXPECTED_TERM_IDS:
        raise SystemExit("Chapter 7 terminology controls are incomplete")
    with corrections_path.open(encoding="utf-8-sig", newline="") as handle:
        corrections = sorted(
            (
                row for row in csv.DictReader(handle)
                if row.get("unit") == "chapter_07_open_balls"
            ),
            key=lambda row: str(row.get("id")),
        )
    if {row.get("id") for row in corrections} != EXPECTED_CORRECTION_IDS:
        raise SystemExit("Chapter 7 source-correction controls are incomplete")
    ...
    for stem, source_name in IMAGE_USES.items():
        ...
        image_assets.append(
            {
                "id": f"o003-gvsu-ch07-{stem.lower().replace('_', '-')}",
    ...
    manifest = {
        ...
        "locale": "id-ID",
        "stable_id_namespace": {
            "prefix": "o003-c90-ch07",
            "source_anchor_prefix": "o003-gvsu-ch07",
            "policy": "locale-neutral deterministic IDs; upstream XML IDs retained where present",
        },
        "unit": {
            "id": "o003-c90-ch07-open-balls-neighborhoods-metric-spaces",
            "sequence": 7,
            "title": "Bola Terbuka dan Lingkungan di Ruang Metrik",
            "prerequisites": [
            ...
            "concepts": [
                "open_balls",
                "neighborhoods",
                "continuity_via_neighborhoods",
                "inverse_image_containment",
                "comparison_of_metrics",
                "weighted_graph_metric",
                "neighborhood_axioms",
                "metric_separation",
            ],
        },
        "component": {
            "id": "o003-c90-ch07-companion",
            "type": "self_study_companion",
            "title": "Pendamping Mandiri Bab 7: Bola Terbuka dan Lingkungan",
            "path": "repo/companion/chapter_07_open_balls_self_study.ptx",
            "fragments": [
                {"path": f"repo/companion/{name}", **file_identity(repo / "companion" / name)}
                for name in FRAGMENTS
            ],
            "entry_alias_map": {
                "path": "repo/backend/chapter_07_entry_aliases.csv",
        ...
        "translated_unit_source_qa": {
            "path": "repo/qa/CHAPTER07_SOURCE_QA.json",
        ...
        "companion_qa": {
            "path": "repo/qa/CHAPTER07_COMPANION_QA.json",
