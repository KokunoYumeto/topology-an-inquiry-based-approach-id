#!/usr/bin/env python3
"""Validate and summarize the cumulative Chapters 1-16 Pages tree.

The Pages overlay is intentionally non-destructive: it contains the exact
final Chapters 1-16 HTML tree, the current PDF reader, and a small frozen set
...
PDF_RUN_1 = ROOT / "qa/CHAPTER16_PDF_RUN1_HASH.json"
PDF_RUN_2 = ROOT / "qa/CHAPTER16_PDF_RUN2_HASH.json"
PDF_RUN_1_CANDIDATE = ROOT / "tmp/pdfs/chapter16-run1.pdf"
DOCS_ROOT = ROOT / "docs"
DOCS_FILE_MANIFEST = ROOT / "qa/CHAPTER16_DOCS_FILE_MANIFEST.json"
DOCS_QA = ROOT / "qa/CHAPTER16_DOCS_QA.json"
PRIOR_DOCS_FILE_MANIFEST = ROOT / "qa/CHAPTER15_DOCS_FILE_MANIFEST.json"
OUTPUT = ROOT / "qa/CHAPTER16_DOCS_MANIFEST.json"

HTML_LABEL = "O003/C90 Chapters 1-16 HTML"
DOCS_LABEL = "O003/C90 Chapters 1-16 GitHub Pages tree"
PRIMARY_ENTRY = "o003-c90-chapters-01-16-reader.html"
PRIMARY_DOWNLOAD = (
    "downloads/topologi-pendekatan-berbasis-inkuiri-bab-01-16-id.pdf"
)
PDF_REPO_PATH = "output/chapters01-16-pdf/chapters_01_16_reader.pdf"
PDF_RUN_1_REPO_PATH = "tmp/pdfs/chapter16-run1.pdf"
...
REQUIRED_DOCS_SURFACES = frozenset(
    {
        ...
        PRIMARY_ENTRY,
        "chap_quotients.html",
        "sec_quotients_exer.html",
        "o003-c90-ch16-companion.html",
        "o003-c90-ch16-mastery.html",
        "external/o003-readable-layout.css",
        PRIMARY_DOWNLOAD,
        "o003-c90-chapters-01-15-reader.html",
        "downloads/topologi-pendekatan-berbasis-inkuiri-bab-01-15-id.pdf",
    }
)

# Chapter 15's detailed Pages manifest is the frozen authority for every byte
# that the Chapter 16 overlay must retain.  Pinning the manifest itself avoids
# accepting a mutually edited historical manifest and docs tree.
PRIOR_MANIFEST_IDENTITY = {
    "bytes": 2_392_724,
    "sha256": "7df4bbf38e8b626c7313ce40e184a55152b761f19e0a11c230d7ad2835cea2ba",
}
...
EXPECTED_HISTORICAL_PATHS = frozenset(
    {
        ...
        *(
            ...
            for chapter in range(2, 16)
        ),
        *(f"o003-c90-ch{chapter:02d}-edition-note.html" for chapter in range(8, 16)),
        *(
...
def validate_pdf_receipts(
    ...
    run_two = read_json(run_two_path, "Chapter 16 PDF run-two receipt")
    ...
    run_one = read_json(run_one_path, "Chapter 16 PDF run-one receipt")
...
def validate_docs_qa(
    ...
    report = read_json(path, "Chapter 16 docs QA")
...
def main() -> int:
    ...
    html_manifest = read_json(html_manifest_path, "Chapter 16 HTML manifest")
    html_rows, html_summary = manifest_rows(
        html_manifest,
        label="Chapter 16 HTML manifest",
        expected_label=HTML_LABEL,
    )
    assert_manifest_matches_tree(html_rows, html_root, "Chapter 16 HTML tree")
    html_by_path = row_map(html_rows)
    require(PRIMARY_ENTRY in html_by_path, "Chapters 1-16 reader entry is absent from HTML")
    ...
    require(
        ...
        "frozen Chapter 15 detailed docs manifest identity differs",
    )
    prior_manifest = read_json(prior_manifest_path, "frozen Chapter 15 docs manifest")
    prior_rows, prior_summary = manifest_rows(
        prior_manifest,
        label="frozen Chapter 15 docs manifest",
        expected_label="O003/C90 Chapters 1-15 GitHub Pages tree",
    )
    require(
        prior_summary == PRIOR_MANIFEST_CENSUS,
        "frozen Chapter 15 docs manifest census differs",
    )
    ...
    require(
        ...
        "the exact Chapter 15 historical-retention set differs",
    )
    historical_rows = [
        prior_by_path[path] for path in sorted(historical_paths, key=str.casefold)
    ]

    docs_manifest = read_json(docs_manifest_path, "Chapter 16 detailed docs manifest")
    docs_rows, docs_summary = manifest_rows(
        docs_manifest,
        label="Chapter 16 detailed docs manifest",
        expected_label=DOCS_LABEL,
    )
    assert_manifest_matches_tree(docs_rows, docs_root, "Chapter 16 docs tree")
    ...
    require(
        ...
        "docs file count is not HTML file count + 32",
    )
    require(
        len(actual_extras) == EXPECTED_EXTRA_FILES,
        "docs overlay does not contain exactly 32 extra files",
    )
    historical_html = sum(
    ...
    output = {
        ...
        "status": "pass",
        "scope": (
            "GitHub Pages tree for the admitted partial Chapters 1-16 boundary "
            "(16 of 20), retaining deliberate historical entry surfaces and "
            "earlier PDFs"
        ),
        **docs_summary,
        "generated_html_tree": {
        ...
        "count_contract": {
            "formula": "docs file count = final HTML file count + 32",
