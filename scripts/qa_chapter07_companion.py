#!/usr/bin/env python3
"""Fail-closed coverage, rights, source-link, and asset audit for Chapter 7."""
...
SOURCE_FILES = (
    "chap_open_balls.ptx",
    "sec_open_balls_intro.ptx",
    "sec_neighborhoods.ptx",
    "sec_cont_neighborhoods.ptx",
    "sec_open_balls_summ.ptx",
    "sec_open_balls_exer.ptx",
)
FILE_GROUPS = {
    "sec_open_balls_intro.ptx": (("intro", 5),),
    "sec_neighborhoods.ptx": (("neighborhood", 3),),
    "sec_cont_neighborhoods.ptx": (
        ("continuity-ball", 3),
        ("continuity-neighborhood", 5),
    ),
    "sec_open_balls_exer.ptx": (("exercise", 24),),
}
FRAGMENTS = (
    "chapter_07_source_guides.ptx",
    "chapter_07_exercise_guides_a.ptx",
    "chapter_07_exercise_guides_b.ptx",
    "chapter_07_mastery.ptx",
)
EXPECTED_AUTHORITY_ORDERED_SHA256 = (
    "3e1fb3bb1afcf443fa540d33dce157395cb8f961a25839c45110271dc45ed850"
)
...
EXPECTED_TERM_IDS = {
    "O003-T013",
    "O003-T046",
    *(f"O003-T{number:03d}" for number in range(98, 103)),
}
EXPECTED_CORRECTION_IDS = {f"O003-C{number:03d}" for number in range(66, 73)}
EXPECTED_SOURCE_PROMPTS = 40
EXPECTED_ACTIVITY_PROMPTS = 16
EXPECTED_EXERCISE_PROMPTS = 24
EXPECTED_MASTERY = 6
...
EXPECTED_INSERTIONS = {
    "sec_neighborhoods.ptx:76:description": "description",
    "sec_cont_neighborhoods.ptx:137:task": "task",
    "sec_open_balls_exer.ptx:83:description": "description",
}
IMAGE_USES = {
    "Open_ball_neighborhood": "sec_neighborhoods.ptx",
    "Graph_metric": "sec_open_balls_exer.ptx",
}
...
def entry_id(group: str, ordinal: int) -> str:
    return f"o003-c90-ch07-{group}-task-{ordinal:02d}"


def expected_source_entry_ids() -> list[str]:
    return [
        entry_id(group, ordinal)
        for groups in FILE_GROUPS.values()
...
def expected_entry_ids() -> list[str]:
    return expected_source_entry_ids() + [
        f"o003-c90-ch07-mastery-{ordinal:02d}"
        for ordinal in range(1, EXPECTED_MASTERY + 1)
    ]


def prompt_nodes(tree: etree._ElementTree, name: str) -> list[Prompt]:
    """Return assessable prompts, including both statements in the malformed shell.

    The frozen authority has one Chapter 7 task with two direct statement children.
    Treating each direct statement as a prompt preserves its two questions while the
    admitted derivative repairs the shell into two consecutive one-statement tasks.
    """
    root = tree.getroot()
    if name != "sec_open_balls_exer.ptx":
        prompts: list[Prompt] = []
...
def source_prompt_mappings(
    ...
    for name, groups in FILE_GROUPS.items():
        ...
        for sequence, (authority, translated) in enumerate(
            ...
            mappings.append(
                {
                    "companion_entry_id": entry_id(group, ordinal),
                    "source_anchor_id": (
                        f"o003-gvsu-ch07-{Path(name).stem}-prompt-{sequence:02d}"
                    ),
                    "group": group,
...
def main() -> int:
    ...
    companion = repo / "companion/chapter_07_open_balls_self_study.ptx"
    report_path = repo / "qa/CHAPTER07_COMPANION_QA.json"
    ...
    if missing:
        raise SystemExit(f"missing required Chapter 7 inputs: {missing}")

    source_qa = json.loads(source_qa_path.read_text(encoding="utf-8"))
    if source_qa.get("status") != "pass":
        failures.append("Chapter 7 source QA is not passing")
    ...
    if set(actual_insertions) != set(EXPECTED_INSERTIONS):
        failures.append("Chapter 7 approved element-insertion closure changed")
    else:
    ...
    if source_qa.get("approved_element_block_moves") not in ([], None):
        failures.append("unexpected approved Chapter 7 element block move")
    if source_qa.get("approved_element_shell_moves") not in ([], None):
        failures.append("unexpected approved Chapter 7 element shell move")
    ...
    if math_changes != [
        ...
        failures.append("Chapter 7 protected-math change is not exactly S to X")
    if set(source_qa.get("approved_external_xref_targets", [])) != EXPECTED_EXTERNAL_XREFS:
        failures.append("Chapter 7 approved external-xref closure changed")
    ...
    if ordered_digest.hexdigest() != EXPECTED_AUTHORITY_ORDERED_SHA256:
        failures.append("frozen Chapter 7 authority name-delimited identity changed")
    if raw_digest.hexdigest() != EXPECTED_AUTHORITY_RAW_SHA256:
        failures.append("frozen Chapter 7 authority raw-concatenation identity changed")
    ...
    root = tree.getroot()
    if root.get(XML_ID) != "o003-c90-ch07-companion":
        failures.append("unexpected Chapter 7 companion root ID")
    if root.get(XML_LANG) != "id-ID":
        failures.append("Chapter 7 companion root is not explicitly id-ID")
    ...
    if {row.get("id") for row in terms} != EXPECTED_TERM_IDS:
        failures.append("Chapter 7 terminology controls are incomplete")
    if any(row.get("status") != "approved" or not row.get("id_ID") for row in terms):
        failures.append("Chapter 7 terminology controls are not fully approved and populated")
    ...
    if {row.get("id") for row in corrections} != EXPECTED_CORRECTION_IDS:
        failures.append("Chapter 7 correction controls O003-C066 through O003-C072 are incomplete")
    if any(row.get("status") != "verified" or not row.get("evidence") for row in corrections):
        failures.append("Chapter 7 correction controls are not verified with evidence")
    ...
    report = {
        ...
        "companion": identity(companion, "companion/chapter_07_open_balls_self_study.ptx"),
        "fragments": [
        ...
        "entries": entries,
        "source_qa": identity(source_qa_path, "qa/CHAPTER07_SOURCE_QA.json"),
        "control_inputs": {
            "terminology": {
    ...
    print(
        json.dumps(
            {
                "report": identity(report_path, "qa/CHAPTER07_COMPANION_QA.json"),
