#!/usr/bin/env python3
"""Fail-closed structural and coverage audit for the Chapter 4 companion."""
...
EXPECTED = {"source_task_guide": 8, "mastery_check": 4}
...
def main() -> int:
    repo = Path(__file__).resolve().parent.parent
    companion = repo / "companion/chapter_04_metric_space_applications_self_study.ptx"
    aliases = repo / "backend/chapter_04_entry_aliases.csv"
    report_path = repo / "qa/CHAPTER04_COMPANION_QA.json"
    parser = etree.XMLParser(resolve_entities=False, no_network=True)
    root = etree.parse(str(companion), parser).getroot()
    failures: list[str] = []

    if root.get(XML_ID) != "o003-c90-ch04-companion":
    ...
    if sequences != list(range(1, sum(EXPECTED.values()) + 1)):
        failures.append(f"alias sequence is not contiguous and ordered: {sequences}")
    ...
    for name in SOURCE_FILES:
        ...
        for node in source_root.iter():
            ...
            if local_name(node) == "task":
                source_task_count += 1
    if source_task_count != EXPECTED["source_task_guide"]:
        failures.append(f"source task count changed: {source_task_count}")
    ...
    for node in elements:
        if local_name(node) in {"m", "me", "men", "md", "mrow", "c", "code", "sage"}:
            continue
        if node.text:
