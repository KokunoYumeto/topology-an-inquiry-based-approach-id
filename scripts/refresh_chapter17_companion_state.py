#!/usr/bin/env python3
"""Seal the complete Chapter 17 staged companion and stable-ID backend."""
...
WRAPPER = COMPANION / "chapter_17_compact_spaces_self_study.ptx"
MASTERY = COMPANION / "chapter_17_mastery.ptx"
GUIDE_FILES = (
    "chapter_17_source_guides_a.ptx",
    "chapter_17_source_guides_b.ptx",
    "chapter_17_source_guides_c.ptx",
    "chapter_17_exercise_guides_b.ptx",
    "chapter_17_exercise_guides_d.ptx",
    "chapter_17_exercise_guides_e.ptx",
)
EXPECTED_WRAPPER_HREFS = tuple(f"./{name}" for name in GUIDE_FILES) + (
    "./chapter_17_mastery.ptx",
)

INVENTORY = BACKEND / "chapter_17_prompt_inventory.json"
PROMPT_MAP = BACKEND / "chapter_17_source_prompt_map.csv"
GROUPING = BACKEND / "chapter_17_grouping_nodes.json"
OCCURRENCE_ALIASES = BACKEND / "chapter_17_occurrence_entry_aliases.csv"
MANIFEST = BACKEND / "chapter_17_companion_manifest.json"
ENTRY_ALIASES = BACKEND / "chapter_17_entry_aliases.csv"
QA_OUTPUT = QA / "CHAPTER17_COMPANION_QA.json"
RECEIPT = QA / "CHAPTER17_COMPANION_BACKEND_RECEIPT.md"
MARKER = BACKEND / ".chapter17_companion_refresh.in_progress.json"

EXPECTED_PHYSICAL_PROMPTS = 76
EXPECTED_CANONICAL_SOURCE = 75
EXPECTED_MASTERY = 8
EXPECTED_GROUPING = 2
EXPECTED_OCCURRENCE_ALIASES = 1
...
def read_prompt_map() -> tuple[list[dict[str, str]], bytes]:
    ...
    if tuple(reader.fieldnames or ()) != PROMPT_FIELDS:
        raise SystemExit(f"wrong Chapter 17 prompt-map fields: {reader.fieldnames}")
    rows = list(reader)
...
def verify_wrapper() -> None:
    ...
    if local_name(root) != "appendix" or root.get(XML_LANG) != "id-ID":
        raise SystemExit("wrong Chapter 17 companion wrapper root or locale")
    hrefs = tuple(document.xpath("//xi:include/@href", namespaces={"xi": XI_NS}))
    if hrefs != EXPECTED_WRAPPER_HREFS:
        raise SystemExit(f"wrong Chapter 17 companion include sequence: {hrefs}")
    prose = text_value(root)
    for required in ("CC BY 4.0", "CC BY-NC-SA 3.0", EXACT_MODEL, "75", "83", "332"):
...
def build_payloads() -> dict[Path, bytes]:
    ...
    expected_mastery = [f"o003-c90-ch17-mastery-{number:02d}" for number in range(1, EXPECTED_MASTERY + 1)]
    if [entry["id"] for entry in mastery_entries] != expected_mastery:
        raise SystemExit("wrong Chapter 17 mastery sequence")
    ...
    if grouping_value.get("grouping_node_count") != EXPECTED_GROUPING or len(grouping_value.get("nodes", [])) != EXPECTED_GROUPING:
        raise SystemExit("wrong Chapter 17 grouping-node count")
    if len(occurrence_alias_rows) != EXPECTED_OCCURRENCE_ALIASES:
        raise SystemExit("wrong Chapter 17 occurrence-alias count")
    ...
    qa = {
        ...
        "inputs": {
            "source_complete_qa": identity(QA / "CHAPTER17_SOURCE_COMPLETE_QA.json"),
            "chapter_schema": identity(QA / "CHAPTER17_SCHEMA_VALIDATION.json"),
            "mastery_schema": identity(QA / "CHAPTER17_MASTERY_SCHEMA_QA.json"),
        },
        "outputs": {
    ...
    receipt_text = f"""# Chapter 17 companion/backend receipt

Status: **pass**

- Source prompts: {EXPECTED_PHYSICAL_PROMPTS} physical occurrences / {EXPECTED_CANONICAL_SOURCE} canonical support entries.
- Explicit duplicate alias: {EXPECTED_OCCURRENCE_ALIASES}; grouping-only nodes: {EXPECTED_GROUPING}.
...
rubric, and complete solution. The source duplicate remains reader-visible and
maps to the earlier canonical support entry. No upstream answer or solution is
