#!/usr/bin/env python3
"""Validate and summarize the cumulative Chapters 1-14 Pages tree.

The Pages overlay is intentionally non-destructive: it contains the exact
final Chapters 1-14 HTML tree, the current PDF reader, and a small frozen set
...
PDF_RUN_1 = ROOT / "qa/CHAPTER14_PDF_RUN1_HASH.json"
PDF_RUN_2 = ROOT / "qa/CHAPTER14_PDF_RUN2_HASH.json"
PDF_RUN_1_CANDIDATE = ROOT / "tmp/pdfs/chapter14-run1.pdf"
DOCS_ROOT = ROOT / "docs"
DOCS_FILE_MANIFEST = ROOT / "qa/CHAPTER14_DOCS_FILE_MANIFEST.json"
DOCS_QA = ROOT / "qa/CHAPTER14_DOCS_QA.json"
PRIOR_DOCS_FILE_MANIFEST = ROOT / "qa/CHAPTER13_DOCS_FILE_MANIFEST.json"
OUTPUT = ROOT / "qa/CHAPTER14_DOCS_MANIFEST.json"

HTML_LABEL = "O003/C90 Chapters 1-14 HTML"
DOCS_LABEL = "O003/C90 Chapters 1-14 GitHub Pages tree"
PRIMARY_ENTRY = "o003-c90-chapters-01-14-reader.html"
PRIMARY_DOWNLOAD = (
    "downloads/topologi-pendekatan-berbasis-inkuiri-bab-01-14-id.pdf"
)
PDF_REPO_PATH = "output/chapters01-14-pdf/chapters_01_14_reader.pdf"
PDF_RUN_1_REPO_PATH = "tmp/pdfs/chapter14-run1.pdf"

REQUIRED_DOCS_SURFACES = frozenset(
    {
        "index.html",
        PRIMARY_ENTRY,
        "chap_continuity_topology.html",
        "sec_cont_top_exer.html",
        "o003-c90-ch14-companion.html",
        "o003-c90-ch14-mastery.html",
        "external/o003-readable-layout.css",
        PRIMARY_DOWNLOAD,
        "o003-c90-chapters-01-13-reader.html",
        "downloads/topologi-pendekatan-berbasis-inkuiri-bab-01-13-id.pdf",
    }
)

# Chapter 13's detailed Pages manifest is the frozen authority for every byte
# that the Chapter 14 overlay must retain.  Pinning the manifest itself avoids
# accepting a mutually edited historical manifest and docs tree.
PRIOR_MANIFEST_IDENTITY = {
    "bytes": 2_034_011,
    "sha256": "aa1e088fdee15d6f53d2150d711baede44b758778cef71749799b44d2f713f09",
}
...
EXPECTED_HISTORICAL_PATHS = frozenset(
    {
        ...
        *(f"o003-c90-ch{chapter:02d}-edition-note.html" for chapter in range(8, 14)),
        *(
...
def validate_pdf_receipts(
    ...
    run_two = read_json(run_two_path, "Chapter 14 PDF run-two receipt")
    ...
    run_one = read_json(run_one_path, "Chapter 14 PDF run-one receipt")
...
def validate_docs_qa(
    ...
    report = read_json(path, "Chapter 14 docs QA")
...
def main() -> int:
    ...
    html_manifest = read_json(html_manifest_path, "Chapter 14 HTML manifest")
    html_rows, html_summary = manifest_rows(
        html_manifest,
        label="Chapter 14 HTML manifest",
        expected_label=HTML_LABEL,
    )
    assert_manifest_matches_tree(html_rows, html_root, "Chapter 14 HTML tree")
    html_by_path = row_map(html_rows)
    require(PRIMARY_ENTRY in html_by_path, "Chapters 1-14 reader entry is absent from HTML")
    ...
    require(
        ...
        "frozen Chapter 13 detailed docs manifest identity differs",
    )
    prior_manifest = read_json(prior_manifest_path, "frozen Chapter 13 docs manifest")
    prior_rows, prior_summary = manifest_rows(
        prior_manifest,
        label="frozen Chapter 13 docs manifest",
        expected_label="O003/C90 Chapters 1-13 GitHub Pages tree",
    )
    require(
        prior_summary == PRIOR_MANIFEST_CENSUS,
        "frozen Chapter 13 docs manifest census differs",
    )
    ...
    require(
        ...
        "the exact Chapter 13 historical-retention set differs",
    )
    historical_rows = [
        prior_by_path[path] for path in sorted(historical_paths, key=str.casefold)
    ]

    docs_manifest = read_json(docs_manifest_path, "Chapter 14 detailed docs manifest")
    docs_rows, docs_summary = manifest_rows(
        docs_manifest,
        label="Chapter 14 detailed docs manifest",
        expected_label=DOCS_LABEL,
    )
    assert_manifest_matches_tree(docs_rows, docs_root, "Chapter 14 docs tree")
    ...
    require(
        ...
        "docs file count is not HTML file count + 26",
    )
    require(
        len(actual_extras) == EXPECTED_EXTRA_FILES,
        "docs overlay does not contain exactly 26 extra files",
    )
    historical_html = sum(
    ...
    output = {
        ...
        "status": "pass",
        "scope": (
            "GitHub Pages tree for the admitted partial Chapters 1-14 boundary "
            "(14 of 20), retaining deliberate historical entry surfaces and "
            "earlier PDFs"
        ),
        **docs_summary,
        "generated_html_tree": {
        ...
        "count_contract": {
            "formula": "docs file count = final HTML file count + 26",
