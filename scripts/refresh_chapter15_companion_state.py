#!/usr/bin/env python3
"""Validate and deterministically seal the complete Chapter 15 companion backend.

The prompt census and pinned-authority locators come from
``build_chapter15_prompt_inventory.py``. This transaction proves that every
...
from build_chapter15_prompt_inventory import (
...
WRAPPER = COMPANION / "chapter_15_subspaces_self_study.ptx"
MASTERY = COMPANION / "chapter_15_mastery.ptx"
INVENTORY = BACKEND / "chapter_15_prompt_inventory.json"
PROMPT_MAP = BACKEND / "chapter_15_source_prompt_map.csv"
GROUPING = BACKEND / "chapter_15_grouping_nodes.json"
MANIFEST = BACKEND / "chapter_15_companion_manifest.json"
ALIASES = BACKEND / "chapter_15_entry_aliases.csv"
QA_OUTPUT = QA / "CHAPTER15_COMPANION_QA.json"
WRAPPER_SCHEMA = QA / "CHAPTER15_COMPANION_WRAPPER_SCHEMA_QA.json"
MASTERY_SCHEMA = QA / "CHAPTER15_MASTERY_SCHEMA_QA.json"
CUMULATIVE_SCHEMA = QA / "CHAPTER15_CUMULATIVE_SCHEMA_QA.json"
SOURCE_QA = QA / "CHAPTER15_SOURCE_COMPLETE_QA.json"
AUTHORITY_AUDIT = LANE / "00_control" / "CHAPTER15_AUTHORITY_AUDIT.md"
CORRECTIONS = LANE / "00_control" / "SOURCE_CORRECTIONS.csv"
TERMINOLOGY = LANE / "00_control" / "TERMINOLOGY.csv"
CUMULATIVE_WRAPPER = ROOT / "source" / "chapters_01_15_reader.ptx"
TRANSACTION_MARKER = BACKEND / ".chapter15_companion_refresh.in_progress.json"
...
EXPECTED_SOURCE_PROMPTS = 30
EXPECTED_MASTERY = 8
EXPECTED_GROUPS = 2

GUIDE_LAYOUT: tuple[tuple[str, tuple[str, ...]], ...] = (
    ("chapter_15_source_guides_a.ptx", tuple(f"o003-c90-ch15-guide-{n:02d}" for n in range(1, 11))),
    ("chapter_15_source_guides_b.ptx", tuple(f"o003-c90-ch15-guide-{n:02d}" for n in range(11, 17))),
    ("chapter_15_exercise_guides_a.ptx", tuple(f"o003-c90-ch15-exer-a-{n:02d}" for n in range(1, 11))),
    ("chapter_15_exercise_guides_b.ptx", tuple(f"o003-c90-ch15-exer-b-{n:02d}" for n in range(1, 5))),
)
...
EXPECTED_MASTERY_IDS = tuple(f"o003-c90-ch15-mastery-{n:02d}" for n in range(1, 9))
EXPECTED_WRAPPER_HREFS = tuple(f"./{name}" for name in GUIDE_FILES) + ("./chapter_15_mastery.ptx",)
...
def parse_mastery() -> list[dict[str, Any]]:
    ...
    if local_name(root) != "section" or root.get(XML_LANG) != "id-ID":
        raise SystemExit("wrong Chapter 15 mastery root or locale")
    exercises = [child for child in root if isinstance(child.tag, str) and local_name(child) == "exercise"]
...
def verify_wrapper() -> list[str]:
    ...
    if local_name(root) != "appendix" or root.get(XML_LANG) != "id-ID":
        raise SystemExit("wrong Chapter 15 companion wrapper root or locale")
    hrefs = tuple(document.xpath("//xi:include/@href", namespaces={"xi": XI[1:].split("}")[0]}))
    if hrefs != EXPECTED_WRAPPER_HREFS:
        raise SystemExit(f"wrong Chapter 15 wrapper includes: {hrefs}")
...
def cumulative_reader_admission() -> dict[str, Any]:
    """Derive Chapter 15 reader admission from the live cumulative wrapper."""
    ...
    main_href = "./chap_subspaces.ptx"
    companion_href = "../companion/chapter_15_subspaces_self_study.ptx"
    main_positions = [
    ...
    admitted = len(main_positions) == 1 and len(companion_positions) == 1
    return {
        "admitted": admitted,
...
def build_payloads() -> tuple[dict[Path, bytes], dict[str, Any]]:
    ...
    if tuple(entry["id"] for entry in source_entries) != EXPECTED_GUIDE_IDS:
        raise SystemExit("companion guide entries do not match the 30-row prompt plan")
    ...
    mastery_schema = read_schema_receipt(MASTERY_SCHEMA, "companion/chapter_15_mastery.ptx")
    cumulative_schema = read_schema_receipt(
        CUMULATIVE_SCHEMA,
        "source/chapters_01_15_reader.ptx",
    )
    source_qa = json.loads(SOURCE_QA.read_text(encoding="utf-8"))
    if source_qa.get("status") != "pass" or source_qa.get("failures") != []:
        raise SystemExit("Chapter 15 source QA is not a pass")
    source_qa_rows = source_qa.get("files")
    if not isinstance(source_qa_rows, list) or len(source_qa_rows) != 7:
        raise SystemExit("Chapter 15 source QA has the wrong file closure")
    for row in source_qa_rows:
        if not isinstance(row, dict) or not isinstance(row.get("file"), str):
            raise SystemExit("Chapter 15 source QA has a malformed file row")
        translated = row.get("translated")
        if not isinstance(translated, dict):
            raise SystemExit(f"Chapter 15 source QA lacks translated identity: {row}")
        current = ROOT / "source" / row["file"]
        if translated.get("bytes") != current.stat().st_size or translated.get("sha256") != digest(current):
            raise SystemExit(f"Chapter 15 source QA identity is stale: {row['file']}")
    ...
    qa_boundary = (
        "chapter_15_complete_companion_backend_reader_admission_pending"
        if reader_admission_pending
        else "chapter_15_complete_companion_backend_reader_admitted"
    )
    manifest_value = {
        "schema_version": 1,
        ...
        "authority": {
            "audit": "00_control/CHAPTER15_AUTHORITY_AUDIT.md",
        ...
        "backend": {
            ...
            "grouping_nodes": {"path": "backend/chapter_15_grouping_nodes.json", **identity_bytes(grouping_payload)},
            "entry_aliases": {"path": "backend/chapter_15_entry_aliases.csv", **identity_bytes(aliases_payload)},
            "inventory": {
                "path": "backend/chapter_15_prompt_inventory.json",
                **identity(INVENTORY),
                "role": (
    ...
    correction_ids = selected_ledger_ids(CORRECTIONS, "O003-C", 182, 187)
    terminology_ids = selected_ledger_ids(TERMINOLOGY, "O003-T", 195, 203)
    qa_value = {
        "schema_version": 1,
        "status": "pass",
        ...
        "source_qa": {"path": "repo/qa/CHAPTER15_SOURCE_COMPLETE_QA.json", **identity(SOURCE_QA), "status": "pass"},
        "schema": {
            "wrapper": wrapper_schema,
            "mastery": mastery_schema,
            "cumulative": cumulative_schema,
        ...
        "authority": {
            ...
            "prompt_inventory": {"path": "repo/backend/chapter_15_prompt_inventory.json", **identity(INVENTORY)},
        ...
        "generated_outputs": {
            "prompt_map": {
                "path": "repo/backend/chapter_15_source_prompt_map.csv",
                "phase": "companion_support_covered",
                **identity_bytes(final_prompt_payload),
            },
            "grouping_nodes": {"path": "repo/backend/chapter_15_grouping_nodes.json", **identity_bytes(grouping_payload)},
            "entry_aliases": {"path": "repo/backend/chapter_15_entry_aliases.csv", **identity_bytes(aliases_payload)},
            "companion_manifest": {"path": "repo/backend/chapter_15_companion_manifest.json", **identity_bytes(manifest_payload)},
        },
        "failures": [],
        "reader_admission_pending": reader_admission_pending,
    }
...
def write_transaction(payloads: dict[Path, bytes]) -> None:
    if TRANSACTION_MARKER.exists():
        raise SystemExit(f"stale Chapter 15 transaction marker requires inspection: {TRANSACTION_MARKER}")
