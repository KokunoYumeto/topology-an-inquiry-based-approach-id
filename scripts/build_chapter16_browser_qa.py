#!/usr/bin/env python3
"""Run deterministic responsive-browser QA for the cumulative Chapter 16 reader.
...
viewports and the Chapter 16 source/companion surfaces, and exercises one
complete Chapter 16 staged disclosure.  It does not build or modify the reader.
...
DEFAULT_SURFACE = ROOT / "output/chapters01-16-html"
DEFAULT_OUTPUT = ROOT / "qa/CHAPTER16_BROWSER_QA.json"
VIEWPORTS = ((1440, 900), (1280, 900), (1024, 900), (768, 900), (390, 900))

ROOT_PAGE = "frontmatter-1.html"
BOOK_PAGE = "o003-c90-chapters-01-16-reader.html"
CHAPTER_PAGE = "chap_quotients.html"
EXERCISES_PAGE = "sec_quotients_exer.html"
COMPANION_PAGE = "o003-c90-ch16-companion.html"
GUIDES_PAGE = "o003-c90-ch16-source-guides-a.html"
MASTERY_PAGE = "o003-c90-ch16-mastery.html"
EDITION_NOTE_PAGE = "o003-c90-ch16-edition-note.html"
GUIDE_ID = "o003-c90-ch16-guide-01"

EVIDENCE_PATHS = (
    ROOT / "qa/CHAPTER16_HTML_MANIFEST.json",
    ROOT / "qa/CHAPTER16_HTML_QA.json",
    ROOT / "qa/CHAPTER16_COMPANION_QA.json",
)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Already-built cumulative Chapter 16 HTML directory.",
    )
    parser.add_argument(
        "--output",
        type=Path,
...
def inspect_identity(page: Any) -> dict[str, Any]:
    """Collect Chapter 16 boundary identity and navigation facts."""
    return page.evaluate(
        ...
        () => {
          ...
          return {
            ...
            rootLinkCount: countHref('o003-c90-chapters-01-16-reader.html'),
            chapterLinkCount: countHref('chap_quotients.html'),
            exercisesLinkCount: countHref('sec_quotients_exer.html'),
            companionLinkCount: countHref('o003-c90-ch16-companion.html'),
            masteryLinkCount: countHref('o003-c90-ch16-mastery.html'),
            editionNoteLinkCount: countHref('o003-c90-ch16-edition-note.html'),
            legacyBoundaryLinks: allHrefs.filter((href) =>
              href.includes('chapters-01-15-reader') ||
              href.includes('ch15-edition-note')
            ).length
          };
        }
        """
...
def validate_identity(
    ...
    checks = {
        ...
        "root_heading_names_chapters_01_16": "Bab 1-16" in root_heading,
        "root_exposes_chapter_16": int(root.get("chapterLinkCount") or 0) > 0,
        ...
        "no_chapter_15_boundary_links": int(root.get("legacyBoundaryLinks") or 0) == 0,
        "chapter_language_id_ID": chapter.get("language") == "id-ID",
        "chapter_book_link_exact": chapter.get("bookLink") == BOOK_PAGE,
        "chapter_heading_is_chapter_16": "Bab 16" in chapter_heading,
        "chapter_heading_names_quotient_spaces": "ruang kuosien" in chapter_heading.lower(),
    ...
    engine.add_failed_checks(failures, "Chapter 16 identity/navigation", checks)
    return {
...
def static_closure(failures: list[str]) -> dict[str, Any]:
    path = ROOT / "qa/CHAPTER16_COMPANION_QA.json"
    ...
    if not passed:
        failures.append(
            f"Chapter 16 static companion closure mismatch: observed {observed}, "
            f"expected {expected}"
        )
    return {
        **observed,
        "source": "qa/CHAPTER16_COMPANION_QA.json and generated companion HTML controls",
        "status": "pass" if passed else "fail",
    }


def evidence_records(failures: list[str]) -> tuple[list[dict[str, Any]], str | None]:
    """Bind the exact Chapter 16 manifest and prerequisite QA receipts."""
    ...
    for path in EVIDENCE_PATHS:
        ...
        payload = engine.load_json(path, failures)
        if path.name == "CHAPTER16_HTML_MANIFEST.json":
            canonical_manifest_sha = payload.get("canonical_manifest_sha256")
            if not canonical_manifest_sha:
                failures.append(
                    "CHAPTER16_HTML_MANIFEST.json lacks canonical_manifest_sha256"
                )
...
def configure_engine() -> None:
    """Bind the maintained Chapter 14 browser engine helpers to Chapter 16."""
    engine.ROOT = ROOT
        ...
        receipt.update(
            {
                ...
                "surface_regression": {
                    ...
                    "chapter_16_desktop": chapter_row,
                    "chapter_16_exercises_tablet": exercises_row,
                    "chapter_16_companion_mobile": companion_row,
                    "chapter_16_mastery_mobile": mastery_row,
                    "all_geometry_checks_pass": all(
                        row.get("status") == "pass" for row in geometry_rows
                    ),
                },
                "chapter_16_semantics": semantics,
                "staged_disclosures": {
                    "chapter_16_live_interaction": disclosure,
                ...
                "checks": {
                    ...
                    "chapter_16_navigation_and_identity": semantics["status"],
