#!/usr/bin/env python3
"""Assemble the full admitted Chapters 1-16 Zenodo package.

This builder consumes only the final cumulative Chapter 16 source manifest and
the admitted Chapter 16 HTML/PDF.  It never reads or advances a historical
...
PDF = ROOT / "output/chapters01-16-pdf/chapters_01_16_reader.pdf"
HTML_ROOT = ROOT / "output/chapters01-16-html"
SOURCE_MANIFEST = ROOT / "qa/CHAPTER16_SOURCE_MANIFEST.json"
HTML_MANIFEST = ROOT / "qa/CHAPTER16_HTML_MANIFEST.json"
AUTHORITY_ARCHIVE = LANE / "authority/archives/gvsuoer-topology-0c2d8f614ef87aa00de373f3418146c2f1d13bb9.zip"

REQUIRED_INVENTORY = {
    "repo/source/chapters_01_16_reader.ptx",
    "repo/companion/chapter_16_quotient_spaces_self_study.ptx",
    "repo/companion/chapter_16_source_guides_a.ptx",
    "repo/companion/chapter_16_source_guides_b.ptx",
    "repo/companion/chapter_16_exercise_guides_a.ptx",
    "repo/companion/chapter_16_exercise_guides_b.ptx",
    "repo/companion/chapter_16_exercise_guides_c.ptx",
    "repo/companion/chapter_16_exercise_guides_d.ptx",
    "repo/companion/chapter_16_mastery.ptx",
    "repo/backend/chapter_16_companion_manifest.json",
    "repo/backend/chapter_16_entry_aliases.csv",
    "repo/backend/chapter_16_occurrence_entry_aliases.csv",
    "repo/backend/chapter_16_source_prompt_map.csv",
    "repo/backend/chapter_16_grouping_nodes.json",
    "repo/backend/chapter_16_prompt_inventory.json",
    "repo/qa/CHAPTER16_SOURCE_COMPLETE_QA.json",
    "repo/qa/CHAPTER16_COMPANION_QA.json",
    "repo/qa/CHAPTER16_COMPANION_WRAPPER_SCHEMA_QA.json",
    "repo/qa/CHAPTER16_MASTERY_SCHEMA_QA.json",
    "repo/qa/CHAPTER16_CUMULATIVE_SCHEMA_QA.json",
    "repo/qa/CHAPTER16_HTML_MANIFEST.json",
    "repo/qa/CHAPTER16_HTML_QA.json",
    "repo/qa/CHAPTER16_BROWSER_QA.json",
    "repo/qa/CHAPTER16_ASSET_PDF_METADATA_QA.json",
    "repo/qa/CHAPTER16_PDF_VISUAL_QA.json",
    "repo/qa/CHAPTER16_BUILD_QA.md",
}
...
def html_entries(
    ...
    for name, path in (
        ...
        ("CHAPTER16_HTML_MANIFEST.json", HTML_MANIFEST),
    ):
...
def source_entries(source_manifest: dict[str, object]) -> dict[str, bytes]:
    ...
    if missing:
        raise RuntimeError(f"final source inventory omits required Chapter 16 files: {sorted(missing)}")
    ...
    manifest_name = f"{prefix}/repo/qa/CHAPTER16_SOURCE_MANIFEST.json"
...
def validate_inputs() -> tuple[dict[str, object], dict[str, object], dict[str, object], int]:
    ...
    if (
        ...
        raise RuntimeError("source manifest is not a truthful admitted Chapter 16 checkpoint")
    ...
    if pdf_row.get("status") != "pass" or pdf_row.get("path") != f"repo/{PDF.relative_to(ROOT).as_posix()}":
        raise RuntimeError("source manifest has not admitted the exact Chapter 16 PDF")
    expected_pdf = {
    ...
    require_identity(PDF, expected_pdf, "Chapter 16 PDF")
    pdf_reader = assert_pdf_privacy(PDF)
    if len(pdf_reader.pages) != pages:
        raise RuntimeError("Chapter 16 PDF page count changed")

    html_row = require_dict(reader.get("html"), "admitted HTML")
    if html_row.get("status") != "deterministic_tree_pass":
        raise RuntimeError("source manifest has not admitted deterministic Chapter 16 HTML")
    expected_html = {
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=ROOT / "output/zenodo-chapters01-16",
...
def main() -> int:
    ...
    try:
        ...
        raise SystemExit(f"Chapter 16 package admission gate failed: {exc}") from exc
    if args.check_only:
        print(json.dumps({
            "status": "pass",
            "partial": True,
    ...
    file_rows = [
        {"path": PDF_NAME, **identity(pdf_target), "role": f"{pages}-page Indonesian cumulative Chapters 1-16 reader PDF"},
        {**html_zip, "role": "admitted cumulative Chapters 1-16 HTML reader and rights notes"},
        {**source_zip, "role": "editable Chapters 1-16 source, companions, final backend, build code, authority identity, and sanitized QA"},
    ...
    package_manifest = {
        ...
        "record": {
            ...
            "language": "ind",
            "completion": {"chapters_verified": 16, "chapters_total": 20, "complete_edition": False, "admitted_boundary_complete": True},
        },
        "authority": {
        ...
        "reader": {
            ...
            "html": expected_html,
            "source_files_translated": 112,
            "source_manifest": {"path": "qa/CHAPTER16_SOURCE_MANIFEST.json", **identity(SOURCE_MANIFEST)},
        },
        "rights": [
        ...
        "caveats": [
            "This is a verified 16-of-20 admitted boundary, not the complete edition.",
