#!/usr/bin/env python3
"""Generate the pinned-schema and ID/xref receipt for completion Module 8."""
...
OUTPUT = ROOT / "qa" / "O003_COMPLETION_MODULE08_SCHEMA_QA.json"
...
def build() -> bytes:
    ...
    if hrefs != ("../completion/module_08_integrated_mastery.ptx",):
        raise SystemExit(f"standalone Module 8 include closure changed: {hrefs}")
    ...
    if not valid or diagnostics:
        raise SystemExit(f"Module 8 schema validation failed: {diagnostics}")
    ...
    if duplicates or unresolved:
        raise SystemExit(f"Module 8 ID/xref gate failed: duplicates={duplicates}, unresolved={unresolved}")
    report = {
        "schema_version": 1,
        "status": "pass",
        "failures": [],
