#!/usr/bin/env python3
"""Assemble the full admitted Chapters 1-12 Zenodo package.

This builder consumes only the final cumulative Chapter 12 source manifest and
the admitted Chapter 12 HTML/PDF.  It never reads or advances the historical
Chapter 12 partial-package directory.  ``--check-only`` validates the frozen
...
BASE = "topologi-pendekatan-berbasis-inkuiri-bab-01-12"
...
PDF = ROOT / "output/chapters01-12-pdf/chapters_01_12_reader.pdf"
HTML_ROOT = ROOT / "output/chapters01-12-html"
SOURCE_MANIFEST = ROOT / "qa/CHAPTER12_SOURCE_MANIFEST.json"
HTML_MANIFEST = ROOT / "qa/CHAPTER12_HTML_MANIFEST.json"
AUTHORITY_ARCHIVE = LANE / "authority/archives/gvsuoer-topology-0c2d8f614ef87aa00de373f3418146c2f1d13bb9.zip"

REQUIRED_INVENTORY = {
    "repo/source/chapters_01_12_reader.ptx",
    "repo/companion/chapter_12_topological_spaces_self_study.ptx",
    "repo/companion/chapter_12_mastery.ptx",
    "repo/backend/chapter_12_companion_manifest.json",
    "repo/backend/chapter_12_entry_aliases.csv",
    "repo/backend/chapter_12_source_prompt_map.csv",
    "repo/backend/chapter_12_grouping_nodes.json",
    "repo/qa/CHAPTER12_SOURCE_QA.json",
    "repo/qa/CHAPTER12_COMPANION_QA.json",
    "repo/qa/CHAPTER12_CUMULATIVE_SCHEMA_QA.json",
...
def html_entries(
    ...
    for name, path in (
        ...
        ("CHAPTER12_HTML_MANIFEST.json", HTML_MANIFEST),
    ):
...
def source_entries(source_manifest: dict[str, object]) -> dict[str, bytes]:
    ...
    if missing:
        raise RuntimeError(f"final source inventory omits required Chapter 12 files: {sorted(missing)}")
...
def validate_inputs() -> tuple[dict[str, object], dict[str, object], dict[str, object], int]:
    ...
    if pdf_row.get("status") != "pass" or pdf_row.get("path") != f"repo/{PDF.relative_to(ROOT).as_posix()}":
        raise RuntimeError("source manifest has not admitted the exact Chapter 12 PDF")
    expected_pdf = {
    ...
    require_identity(PDF, expected_pdf, "Chapter 12 PDF")
    pdf_reader = assert_pdf_privacy(PDF)
    if len(pdf_reader.pages) != pages:
        raise RuntimeError("Chapter 12 PDF page count changed")

    html_row = require_dict(reader.get("html"), "admitted HTML")
    if html_row.get("status") != "deterministic_tree_pass":
        raise RuntimeError("source manifest has not admitted deterministic Chapter 12 HTML")
    expected_html = {
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=ROOT / "output/zenodo-chapters01-12",
...
def main() -> int:
    ...
    try:
        ...
        raise SystemExit(f"Chapter 12 package admission gate failed: {exc}") from exc
    if args.check_only:
        print(json.dumps({
            "status": "pass",
            "partial": True,
    ...
    package_manifest = {
        ...
        "record": {
            ...
            "language": "ind",
            "completion": {"chapters_verified": 12, "chapters_total": 20, "complete_edition": False, "admitted_boundary_complete": True},
        },
        "authority": {
        ...
        "reader": {
            ...
            "html": expected_html,
            "source_files_translated": 81,
            "source_manifest": {"path": "qa/CHAPTER12_SOURCE_MANIFEST.json", **identity(SOURCE_MANIFEST)},
        },
        "rights": [
