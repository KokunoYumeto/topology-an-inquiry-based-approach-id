#!/usr/bin/env python3
"""Validate and deterministically seal the complete Chapter 14 companion backend.

The prompt census and pinned-authority locators come from
``build_chapter14_prompt_inventory.py``. This transaction proves that every
...
from build_chapter14_prompt_inventory import (
...
WRAPPER = COMPANION / "chapter_14_continuity_homeomorphisms_self_study.ptx"
MASTERY = COMPANION / "chapter_14_mastery.ptx"
INVENTORY = BACKEND / "chapter_14_prompt_inventory.json"
PROMPT_MAP = BACKEND / "chapter_14_source_prompt_map.csv"
GROUPING = BACKEND / "chapter_14_grouping_nodes.json"
MANIFEST = BACKEND / "chapter_14_companion_manifest.json"
ALIASES = BACKEND / "chapter_14_entry_aliases.csv"
QA_OUTPUT = QA / "CHAPTER14_COMPANION_QA.json"
WRAPPER_SCHEMA = QA / "CHAPTER14_COMPANION_WRAPPER_SCHEMA_QA.json"
MASTERY_SCHEMA = QA / "CHAPTER14_MASTERY_SCHEMA_QA.json"
SOURCE_QA = QA / "CHAPTER14_SOURCE_COMPLETE_QA.json"
AUTHORITY_AUDIT = LANE / "00_control" / "CHAPTER14_AUTHORITY_AUDIT.md"
CORRECTIONS = LANE / "00_control" / "SOURCE_CORRECTIONS.csv"
TERMINOLOGY = LANE / "00_control" / "TERMINOLOGY.csv"
CUMULATIVE_WRAPPER = ROOT / "source" / "chapters_01_14_reader.ptx"
TRANSACTION_MARKER = BACKEND / ".chapter14_companion_refresh.in_progress.json"
...
EXPECTED_SOURCE_PROMPTS = 81
EXPECTED_MASTERY = 8
EXPECTED_GROUPS = 3

GUIDE_LAYOUT: tuple[tuple[str, tuple[str, ...]], ...] = (
    ("chapter_14_source_guides_a.ptx", tuple(f"o003-c90-ch14-guide-{n:02d}" for n in range(1, 11))),
    ("chapter_14_source_guides_b.ptx", tuple(f"o003-c90-ch14-guide-{n:02d}" for n in range(11, 21))),
    ("chapter_14_source_guides_c.ptx", tuple(f"o003-c90-ch14-guide-{n:02d}" for n in range(21, 26))),
    ("chapter_14_exercise_guides_a.ptx", tuple(f"o003-c90-ch14-exer-a-{n:02d}" for n in range(1, 11))),
    ("chapter_14_exercise_guides_b.ptx", tuple(f"o003-c90-ch14-exer-b-{n:02d}" for n in range(1, 11))),
    ("chapter_14_exercise_guides_c.ptx", tuple(f"o003-c90-ch14-exer-c-{n:02d}" for n in range(1, 11))),
    ("chapter_14_exercise_guides_d.ptx", tuple(f"o003-c90-ch14-exer-d-{n:02d}" for n in range(1, 11))),
    ("chapter_14_exercise_guides_e.ptx", tuple(f"o003-c90-ch14-exer-e-{n:02d}" for n in range(1, 11))),
    ("chapter_14_exercise_guides_f.ptx", tuple(f"o003-c90-ch14-exer-f-{n:02d}" for n in range(1, 7))),
)
...
EXPECTED_MASTERY_IDS = tuple(f"o003-c90-ch14-mastery-{n:02d}" for n in range(1, 9))
EXPECTED_WRAPPER_HREFS = tuple(f"./{name}" for name in GUIDE_FILES) + ("./chapter_14_mastery.ptx",)
...
def parse_mastery() -> list[dict[str, Any]]:
    ...
    if local_name(root) != "section" or root.get(XML_LANG) != "id-ID":
        raise SystemExit("wrong Chapter 14 mastery root or locale")
    exercises = [child for child in root if isinstance(child.tag, str) and local_name(child) == "exercise"]
...
def verify_wrapper() -> list[str]:
    ...
    if local_name(root) != "appendix" or root.get(XML_LANG) != "id-ID":
        raise SystemExit("wrong Chapter 14 companion wrapper root or locale")
    hrefs = tuple(document.xpath("//xi:include/@href", namespaces={"xi": XI[1:].split("}")[0]}))
    if hrefs != EXPECTED_WRAPPER_HREFS:
        raise SystemExit(f"wrong Chapter 14 wrapper includes: {hrefs}")
    visible = " ".join("".join(root.itertext()).split())
    for required in ("81", "Tiga", "CC BY 4.0", "CC BY-NC-SA 3.0", "bukan teks", "solusi resmi"):
...
def build_payloads() -> tuple[dict[Path, bytes], dict[str, Any]]:
    ...
    if tuple(entry["id"] for entry in source_entries) != EXPECTED_GUIDE_IDS:
        raise SystemExit("companion guide entries do not match the 81-row prompt plan")
    ...
    mastery_schema = read_schema_receipt(MASTERY_SCHEMA, "companion/chapter_14_mastery.ptx")
    source_qa = json.loads(SOURCE_QA.read_text(encoding="utf-8"))
    if source_qa.get("status") != "pass" or source_qa.get("failures") != []:
        raise SystemExit("Chapter 14 source QA is not a pass")
    source_qa_rows = source_qa.get("files")
    if not isinstance(source_qa_rows, list) or len(source_qa_rows) != 8:
        raise SystemExit("Chapter 14 source QA has the wrong file closure")
    for row in source_qa_rows:
        if not isinstance(row, dict) or not isinstance(row.get("file"), str):
            raise SystemExit("Chapter 14 source QA has a malformed file row")
        translated = row.get("translated")
        if not isinstance(translated, dict):
            raise SystemExit(f"Chapter 14 source QA lacks translated identity: {row}")
        current = ROOT / "source" / row["file"]
        if translated.get("bytes") != current.stat().st_size or translated.get("sha256") != digest(current):
            raise SystemExit(f"Chapter 14 source QA identity is stale: {row['file']}")
    ...
    manifest_value = {
        ...
        "unit": {
            ...
            "title": "Kekontinuan dan Homeomorfisme",
            "sequence": 14,
        },
        "authority": {
            "audit": "00_control/CHAPTER14_AUTHORITY_AUDIT.md",
        ...
        "backend": {
            "prompt_map": {"path": "backend/chapter_14_source_prompt_map.csv", **identity_bytes(final_prompt_payload)},
            "grouping_nodes": {"path": "backend/chapter_14_grouping_nodes.json", **identity_bytes(grouping_payload)},
            "entry_aliases": {"path": "backend/chapter_14_entry_aliases.csv", **identity_bytes(aliases_payload)},
            "inventory": {
                "path": "backend/chapter_14_prompt_inventory.json",
                **identity(INVENTORY),
    ...
    correction_ids = selected_ledger_ids(CORRECTIONS, "O003-C", 162, 181)
    terminology_ids = selected_ledger_ids(TERMINOLOGY, "O003-T", 167, 194)
    qa_value = {
        "schema_version": 1,
        "status": "pass",
        "boundary": "chapter_14_complete_companion_backend_reader_admission_pending",
        "source_prompt_total": EXPECTED_SOURCE_PROMPTS,
        ...
        "source_qa": {"path": "repo/qa/CHAPTER14_SOURCE_COMPLETE_QA.json", **identity(SOURCE_QA), "status": "pass"},
        "schema": {"wrapper": wrapper_schema, "mastery": mastery_schema},
        "authority": {
            "commit": FROZEN_AUTHORITY_COMMIT,
            "canonical_prompt_mapping_sha256": inventory_summary["canonical_prompt_mapping_sha256"],
            "prompt_inventory": {"path": "repo/backend/chapter_14_prompt_inventory.json", **identity(INVENTORY)},
        ...
        "generated_outputs": {
            "prompt_map": {"path": "repo/backend/chapter_14_source_prompt_map.csv", **identity_bytes(final_prompt_payload)},
            "grouping_nodes": {"path": "repo/backend/chapter_14_grouping_nodes.json", **identity_bytes(grouping_payload)},
            "entry_aliases": {"path": "repo/backend/chapter_14_entry_aliases.csv", **identity_bytes(aliases_payload)},
            "companion_manifest": {"path": "repo/backend/chapter_14_companion_manifest.json", **identity_bytes(manifest_payload)},
        },
        "failures": [],
        "reader_admission_pending": True,
    }
...
def write_transaction(payloads: dict[Path, bytes]) -> None:
    if TRANSACTION_MARKER.exists():
        raise SystemExit(f"stale Chapter 14 transaction marker requires inspection: {TRANSACTION_MARKER}")
