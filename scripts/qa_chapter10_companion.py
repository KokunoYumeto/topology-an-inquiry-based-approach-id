#!/usr/bin/env python3
"""Fail-closed coverage, rights, and source-link audit for Chapter 10."""
...
SOURCE_FILES = (
    ...
    "sec_closed_sets_intro.ptx",
    "sec_closed_sets_metric.ptx",
    "sec_cont_closed_sets.ptx",
    "sec_limit_bound_iso_seq.ptx",
    "sec_limit_closed.ptx",
    "sec_set_closure.ptx",
    "sec_closed_limits_seq.ptx",
    "sec_closed_set_summ.ptx",
    "sec_closed_set_exer.ptx",
)
FILE_GROUPS = {
    "sec_closed_sets_intro.ptx": (("sec_closed_sets_intro", 9),),
    "sec_closed_sets_metric.ptx": (("sec_closed_sets_metric", 4),),
    "sec_cont_closed_sets.ptx": (("sec_cont_closed_sets", 7),),
    "sec_limit_bound_iso_seq.ptx": (("sec_limit_bound_iso_seq", 4),),
    "sec_limit_closed.ptx": (("sec_limit_closed", 4),),
    "sec_set_closure.ptx": (("sec_set_closure", 8),),
    "sec_closed_limits_seq.ptx": (("sec_closed_limits_seq", 7),),
    "sec_closed_set_exer.ptx": (("sec_closed_set_exer", 37),),
}
FRAGMENTS = (
    "chapter_10_source_guides_a.ptx",
    "chapter_10_source_guides_b.ptx",
    "chapter_10_exercise_guides_a.ptx",
    "chapter_10_exercise_guides_b.ptx",
    "chapter_10_mastery.ptx",
)
...
EXPECTED_TERM_IDS = {f"O003-T{number:03d}" for number in range(129, 133)}
EXPECTED_CORRECTION_IDS = {f"O003-C{number:03d}" for number in range(97, 113)}
EXPECTED_SOURCE_PROMPTS = 80
EXPECTED_ACTIVITY_PROMPTS = 43
EXPECTED_EXERCISE_PROMPTS = 37
EXPECTED_MASTERY = 8
EXPECTED_GROUPING_TASKS = 5
REMOTE_TAGS = {"url", "video", "interactive", "sage", "webwork", "iframe"}
EXPECTED_MATH_CHANGES = [
    {
        "authority": "<m>x</m>",
        "key": "sec_limit_bound_iso_seq.ptx:59",
        "translated": "<m>b</m>",
    },
    {
        "authority": "<m>X \\in \\overline{A} \\cap \\overline{X \\setminus A}</m>",
        "key": "sec_set_closure.ptx:122",
        "translated": "<m>x \\in \\overline{A} \\cap \\overline{X \\setminus A}</m>",
    },
    {
        "authority": "<m>X \\in \\overline{A} \\cap \\overline{X \\setminus A}</m>",
        "key": "sec_set_closure.ptx:131",
        "translated": "<m>x \\in \\overline{A} \\cap \\overline{X \\setminus A}</m>",
    },
    {
        "authority": "<m>a_n =\\frac{1}{n}</m>",
        "key": "sec_closed_limits_seq.ptx:9",
        "translated": "<m>a_n =\\frac{1}{n+1}</m>",
    },
    {
        "authority": "<m>=(\\R, d_E)</m>",
        "key": "sec_closed_set_summ.ptx:22",
        "translated": "<m>(\\R, d_E)</m>",
    },
    {
        "authority": "<m>A = \\left\\{\\frac{1}{n} \\ \\left| \\right. \\in \\Z^+\\right\\}</m>",
        "key": "sec_closed_set_exer.ptx:11",
        "translated": "<m>A = \\left\\{\\frac{1}{n} \\ \\left| \\right. n \\in \\Z^+\\right\\}</m>",
    },
    {
        "authority": "<m>A \\cup A^i = A \\cup A'</m>",
        "key": "sec_closed_set_exer.ptx:34",
        "translated": "<m>A' \\cup A^i = A \\cup A'</m>",
    },
    {
        "authority": "<me> d(a,X) = \\inf\\{d(x,a) \\mid a \\in A\\} </me>",
        "key": "sec_closed_set_exer.ptx:85",
...
def guide_id(sequence: int) -> str:
    return f"o003-c90-ch10-guide-{sequence:02d}"
...
def expected_entry_ids() -> list[str]:
    return expected_source_entry_ids() + [
        f"o003-c90-ch10-mastery-{ordinal:02d}" for ordinal in range(1, EXPECTED_MASTERY + 1)
    ]
...
def prompt_nodes(tree: etree._ElementTree, name: str) -> list[Prompt]:
    ...
    root = tree.getroot()
    if name != "sec_closed_set_exer.ptx":
        return [
    ...
    for name, groups in FILE_GROUPS.items():
        ...
        for sequence, (authority, translated) in enumerate(zip(authority_prompts, translated_prompts), start=1):
            ...
            mappings.append({
                ...
                "source_anchor_id": f"o003-gvsu-ch10-{Path(name).stem}-prompt-{sequence:02d}",
                "group": group,
...
def main() -> int:
    ...
    companion = repo / "companion/chapter_10_closed_sets_self_study.ptx"
    report_path = repo / "qa/CHAPTER10_COMPANION_QA.json"
    ...
    for key in (
        ...
        "approved_element_insertions",
        "approved_element_shell_moves",
        "approved_external_xref_targets",
    ):
        if source_qa.get(key) not in ([], None):
            failures.append(f"unexpected Chapter 10 source QA closure: {key}")
    if source_qa.get("approved_math_changes") != EXPECTED_MATH_CHANGES:
        failures.append("Chapter 10 protected-math repair closure changed")
    ...
    if ordered_digest.hexdigest() != EXPECTED_AUTHORITY_ORDERED_SHA256:
        failures.append("frozen Chapter 10 authority name-delimited identity changed")
    if raw_digest.hexdigest() != EXPECTED_AUTHORITY_RAW_SHA256:
        failures.append("frozen Chapter 10 authority raw-concatenation identity changed")
    ...
    root = tree.getroot()
    if root.get(XML_ID) != "o003-c90-ch10-companion":
        failures.append("unexpected Chapter 10 companion root ID")
    if root.get(XML_LANG) != "id-ID":
        failures.append("Chapter 10 companion root is not explicitly id-ID")
    ...
    if {row.get("id") for row in terms} != EXPECTED_TERM_IDS:
        failures.append("Chapter 10 terminology controls are incomplete")
    if any(row.get("status") != "approved" or not row.get("id_ID") for row in terms):
        failures.append("Chapter 10 terminology controls are not fully approved and populated")
    with corrections_path.open(encoding="utf-8-sig", newline="") as handle:
        corrections = [row for row in csv.DictReader(handle) if row.get("unit") == "chapter_10_closed_sets_metric"]
    if {row.get("id") for row in corrections} != EXPECTED_CORRECTION_IDS:
        failures.append("Chapter 10 correction controls O003-C097 through O003-C112 are incomplete")
    if any(row.get("status") != "verified" or not row.get("evidence") for row in corrections):
        failures.append("Chapter 10 correction statuses or evidence differ from the frozen ledger")
    ...
    report = {
        ...
        "companion": identity(companion, "companion/chapter_10_closed_sets_self_study.ptx"),
        ...
        "entries": entries,
        "source_qa": identity(source_qa_path, "qa/CHAPTER10_SOURCE_QA.json"),
        "control_inputs": {
    ...
    print(json.dumps({
        "report": identity(report_path, "qa/CHAPTER10_COMPANION_QA.json"),
