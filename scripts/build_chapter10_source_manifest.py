#!/usr/bin/env python3
"""Build the fail-closed cumulative Chapters 1-10 source/artifact manifest.

The source, companion, backend, deterministic reader artifacts, browser/PDF
evidence, documentation receipts, and build receipt are mandatory.  The
manifest also freezes the exact source-package inventory used by publication.
"""
...
from qa_chapter10_companion import (
...
EXPECTED_INSERTIONS: set[str] = set()
LANE = ROOT.parent
OUTPUT = ROOT / "qa" / "CHAPTER10_SOURCE_MANIFEST.json"
AUTHORITY_SOURCE = (
    LANE
    / "authority/gvsu-pinned/topology-0c2d8f614ef87aa00de373f3418146c2f1d13bb9/source"
)
PRIOR_MANIFEST = "qa/CHAPTER09_SOURCE_MANIFEST.json"
PRIOR_MANIFEST_IDENTITY = {
    "bytes": 60_728,
    "sha256": "a731db7e0a89f68a638208055997f5f51a167dd2a552f7ff69d3a3208527480d",
}
PRIOR_CHAPTERS = tuple(f"chapter_{number:02d}" for number in range(1, 10))
COMPANION_WRAPPER = "companion/chapter_10_closed_sets_self_study.ptx"
COMPANION_FRAGMENTS = tuple(f"companion/{name}" for name in FRAGMENTS)
ALIAS_MAP = "backend/chapter_10_entry_aliases.csv"
BACKEND_MANIFEST = "backend/chapter_10_companion_manifest.json"
NEW_ADDITIVE = (
    COMPANION_WRAPPER,
    *COMPANION_FRAGMENTS,
    ALIAS_MAP,
...
CHAPTER_10_IMPLEMENTATION = (
    "project.ptx",
    "scripts/qa_chapter10_companion.py",
    "scripts/build_chapter10_backend.py",
    "scripts/build_chapter10_source_manifest.py",
    "scripts/build_zenodo_chapter10_package.py",
    "scripts/qa_html_tree.py",
    "source/chapters_01_10_reader.ptx",
)
EVOLVING_PRIOR_IMPLEMENTATION = {
    "README.md": "advance the truthful cumulative reader boundary to Chapters 1-10",
    "LICENSES.md": "cumulative Chapter 10 collection-rights preservation",
    "assets/o003-readable-layout.css": "cumulative reader layout evolution",
    "companion/RIGHTS.md": "cumulative Chapter 10 companion rights preservation",
    "project.ptx": "addition of cumulative Chapters 1-10 build targets",
    "scripts/qa_source_translation.py": "Chapter 10 source topology and approved-change validation",
...
EVOLVING_PRIOR_SOURCE: dict[str, str] = {}
CONTROL_INPUTS = (
...
PRIOR_PUBLICATION_RECEIPTS = (
    "qa/CHAPTER09_GITHUB_PUBLICATION_RECEIPT.md",
    "qa/CHAPTER09_ZENODO_PUBLICATION_RECEIPT.md",
    "qa/CHAPTER09_FIGSHARE_PUBLICATION_RECEIPT.md",
)
DOCS_EVIDENCE = (
    "qa/CHAPTER10_DOCS_MANIFEST.json",
    "qa/CHAPTER10_DOCS_QA.json",
    "qa/CHAPTER10_BUILD_QA.md",
)
EXPECTED_PRIOR_SOURCE_FILES = 56
EXPECTED_CHAPTER_SOURCE_FILES = 10
EXPECTED_TOTAL_SOURCE_FILES = 66
EXPECTED_COMPANION_FRAGMENTS = 5
EXPECTED_SOURCE_GUIDES = 80
EXPECTED_MASTERY_CHECKS = 8
EXPECTED_TOTAL_ENTRIES = 88
EXPECTED_CORRECTION_RECORDS = 16
EXPECTED_TERM_ID_SET = {f"O003-T{number:03d}" for number in range(129, 133)}
EXPECTED_CORRECTION_ID_SET = {f"O003-C{number:03d}" for number in range(97, 113)}
HTML_RUN_1 = "qa/CHAPTER10_HTML_MANIFEST_RUN1.json"
HTML_RUN_2 = "qa/CHAPTER10_HTML_MANIFEST_RUN2.json"
HTML_CANONICAL = "qa/CHAPTER10_HTML_MANIFEST.json"
HTML_QA = "qa/CHAPTER10_HTML_QA.json"
HTML_ROOT = "output/chapters01-10-html"
PDF_RELATIVE = "output/chapters01-10-pdf/chapters_01_10_reader.pdf"
PDF_EVIDENCE = {
    "deterministic_run_1": "qa/CHAPTER10_PDF_RUN1_HASH.json",
    "deterministic_run_2": "qa/CHAPTER10_PDF_RUN2_HASH.json",
    "structure_qa": "qa/CHAPTER10_PDF_STRUCTURE.json",
    "visual_qa": "qa/CHAPTER10_PDF_VISUAL_QA.json",
}
AUTHORITY_ARCHIVE = (
    LANE
...
def validate_html_tree(html_root: Path, manifest: dict[str, Any]) -> tuple[int, int, int]:
    ...
    if set(actual) != set(expected):
        ...
        raise RuntimeError(f"canonical Chapter 10 HTML closure differs: missing={missing}, extra={extra}")
    total_bytes = 0
    html_files = 0
...
def main() -> int:
    ...
    assert_identity("pinned Chapter 9 source manifest", PRIOR_MANIFEST_IDENTITY, identity(prior_path))
    prior = read_json(PRIOR_MANIFEST)
    require_pass(prior, "Chapter 9 cumulative source manifest")
    if prior.get("boundary") != "chapters_01_09_with_separately_licensed_self_study_companions":
        raise RuntimeError("unexpected Chapter 9 admission boundary")
    if len(SOURCE_FILES) != EXPECTED_CHAPTER_SOURCE_FILES:
        raise RuntimeError(
            f"Chapter 10 source closure is not exactly {EXPECTED_CHAPTER_SOURCE_FILES} files"
        )
    if len(FRAGMENTS) != EXPECTED_COMPANION_FRAGMENTS:
        raise RuntimeError(
            f"Chapter 10 companion closure is not exactly "
            f"{EXPECTED_COMPANION_FRAGMENTS} fragments"
        )
    if set(EXPECTED_TERM_IDS) != EXPECTED_TERM_ID_SET:
        raise RuntimeError("Chapter 10 controlled-term ID closure changed")
    if set(EXPECTED_CORRECTION_IDS) != EXPECTED_CORRECTION_ID_SET:
        raise RuntimeError("Chapter 10 correction ID closure changed")
    ...
    if set(prior_source_names).intersection(SOURCE_FILES):
        raise RuntimeError("Chapter 10 source closure overlaps prior chapters")

    source_qa = read_json("qa/CHAPTER10_SOURCE_QA.json")
    companion_qa = read_json("qa/CHAPTER10_COMPANION_QA.json")
    backend = read_json(BACKEND_MANIFEST)
    require_pass(source_qa, "Chapter 10 source QA")
    require_pass(companion_qa, "Chapter 10 companion QA")
    if source_qa.get("failures") != [] or companion_qa.get("failures") != []:
        raise RuntimeError("Chapter 10 source or companion QA records failures")

    source_qa_rows = {
        str(row.get("file")): require_dict(row, "Chapter 10 source-QA file")
        for row in require_list(source_qa.get("files"), "Chapter 10 source-QA files")
    }
    if set(source_qa_rows) != set(SOURCE_FILES):
        raise RuntimeError("Chapter 10 source-QA file closure changed")
    ...
    for name in SOURCE_FILES:
        ...
        assert_identity(f"Chapter 10 authority {name}", row.get("authority"), authority)
        assert_identity(f"Chapter 10 translated source {name}", row.get("translated"), translated)
    ...
    if authority_combined != EXPECTED_AUTHORITY_ORDERED_SHA256:
        raise RuntimeError("frozen Chapter 10 ordered authority closure changed")
    if authority_raw != EXPECTED_AUTHORITY_RAW_SHA256:
        raise RuntimeError("frozen Chapter 10 raw authority closure changed")
    ...
    if source_qa.get("combined_translated_sha256") != chapter_combined:
        raise RuntimeError("Chapter 10 source-QA combined translated identity is stale")
    if set(source_qa.get("approved_external_xref_targets", [])) != EXPECTED_EXTERNAL_XREFS:
        raise RuntimeError("Chapter 10 external-xref admission changed")
    insertion_rows = {
        row.get("key"): row
        for row in require_list(source_qa.get("approved_element_insertions"), "Chapter 10 insertions")
    }
    if set(insertion_rows) != set(EXPECTED_INSERTIONS):
        raise RuntimeError("Chapter 10 approved insertion closure changed")
    math_rows = {
        row.get("key"): row
        for row in require_list(source_qa.get("approved_math_changes"), "Chapter 10 math changes")
    }
    expected_math_rows = {row["key"]: row for row in EXPECTED_MATH_CHANGES}
    if math_rows != expected_math_rows:
        raise RuntimeError("Chapter 10 approved protected-math repair closure changed")

    companion_row = file_row(COMPANION_WRAPPER)
    source_qa_row = file_row("qa/CHAPTER10_SOURCE_QA.json")
    companion_qa_row = file_row("qa/CHAPTER10_COMPANION_QA.json")
    backend_row = file_row(BACKEND_MANIFEST)
    alias_row = file_row(ALIAS_MAP)
    assert_identity("Chapter 10 companion", companion_qa.get("companion"), companion_row)
    assert_identity("Chapter 10 source-QA receipt", companion_qa.get("source_qa"), source_qa_row)
    fragment_rows = rows_by_path(companion_qa.get("fragments"), "Chapter 10 fragments")
    if set(fragment_rows) != set(COMPANION_FRAGMENTS):
        raise RuntimeError("Chapter 10 companion fragment closure changed")
    for relative, row in fragment_rows.items():
        assert_identity(f"Chapter 10 fragment {relative}", row, file_row(relative))

    if backend.get("schema_version") != "2.0.0" or backend.get("locale") != "id-ID":
        raise RuntimeError("Chapter 10 backend schema or locale changed")
    component = require_dict(backend.get("component"), "Chapter 10 backend component")
    assert_path("Chapter 10 backend component", component.get("path"), f"repo/{COMPANION_WRAPPER}")
    assert_identity("Chapter 10 backend component", component.get("identity"), companion_row)
    backend_alias = require_dict(component.get("entry_alias_map"), "Chapter 10 backend aliases")
    assert_path("Chapter 10 backend aliases", backend_alias.get("path"), f"repo/{ALIAS_MAP}")
    assert_identity("Chapter 10 backend aliases", backend_alias, alias_row)
    backend_source_qa = require_dict(backend.get("translated_unit_source_qa"), "backend source QA")
    assert_path("backend source QA", backend_source_qa.get("path"), "repo/qa/CHAPTER10_SOURCE_QA.json")
    ...
    if backend_source_qa.get("combined_translated_sha256") != chapter_combined:
        raise RuntimeError("backend Chapter 10 translated-source identity is stale")
    backend_companion_qa = require_dict(backend.get("companion_qa"), "backend companion QA")
    assert_path("backend companion QA", backend_companion_qa.get("path"), "repo/qa/CHAPTER10_COMPANION_QA.json")
    ...
    if entry_counts.get("source_prompt_guide") != EXPECTED_SOURCE_GUIDES:
        raise RuntimeError("Chapter 10 source-guide count changed")
    activity_guides = int(entry_counts.get("activity_or_task_guide", -1))
    exercise_guides = int(entry_counts.get("exercise_prompt_guide", -1))
    if activity_guides + exercise_guides != EXPECTED_SOURCE_GUIDES:
        raise RuntimeError("Chapter 10 activity/exercise guide partition changed")
    if entry_counts.get("mastery_check") != EXPECTED_MASTERY_CHECKS:
        raise RuntimeError("Chapter 10 mastery-check count changed")
    if entry_counts.get("total") != EXPECTED_TOTAL_ENTRIES:
        raise RuntimeError("Chapter 10 companion total-entry count changed")
    ...
    for kind in ("statement", "hint", "answer", "solution"):
        if surface_counts.get(kind) != EXPECTED_TOTAL_ENTRIES:
            raise RuntimeError(f"Chapter 10 companion lacks complete {kind} surfaces")
    for kind in ("hint", "answer", "solution"):
        if reveal_counts.get(kind) != EXPECTED_TOTAL_ENTRIES:
            raise RuntimeError(f"Chapter 10 companion lacks complete {kind} reveals")
    ...
    for key, expected in expected_coverage.items():
        if coverage.get(key) != expected:
            raise RuntimeError(f"Chapter 10 backend coverage is stale for {key}")
    if (
        int(coverage.get("activity_or_task_guides", -1))
        + int(coverage.get("exercise_prompt_guides", -1))
        != EXPECTED_SOURCE_GUIDES
    ):
        raise RuntimeError("Chapter 10 backend guide partition changed")
    if coverage.get("all_entries_have_statement_hint_answer_solution") is not True:
        raise RuntimeError("Chapter 10 backend complete-surface contract is false")
    ...
    if inherited_additive_paths.intersection(NEW_ADDITIVE):
        raise RuntimeError("Chapter 10 additive component duplicates a prior component")

    prior_controls = rows_by_path(prior.get("control_inputs"), "prior control inputs")
    if set(prior_controls) != {*CONTROL_INPUTS, TERMINOLOGY_AUDIT}:
        raise RuntimeError("prior control-input closure changed")
    assert_identity(
        "inherited terminology audit",
        prior_controls[TERMINOLOGY_AUDIT],
    ...
    for relative in CHAPTER_10_IMPLEMENTATION:
    ...
    require_pass(html_qa, "Chapter 10 HTML QA")
    if html_qa.get("failures") != []:
        raise RuntimeError("Chapter 10 HTML QA records failures")
    identity_keys = ("file_count", "html_files", "total_bytes", "canonical_manifest_sha256")
    if html_run_1.get("files") != html_run_2.get("files"):
        raise RuntimeError("Chapter 10 deterministic HTML runs have different file identities")
    for key in identity_keys:
        if html_run_1.get(key) != html_run_2.get(key) or html_qa.get(key) != html_run_2.get(key):
            raise RuntimeError(f"Chapter 10 HTML evidence differs for {key}")
    ...
    if html_manifest.get("files") != html_run_2.get("files"):
        raise RuntimeError("canonical Chapter 10 HTML identities differ from deterministic run 2")
    for key in identity_keys:
        if html_manifest.get(key) != html_run_2.get(key):
            raise RuntimeError(f"canonical Chapter 10 HTML differs for {key}")
    html_file_count, html_files, html_bytes = validate_html_tree(ROOT / HTML_ROOT, html_manifest)

    browser_relative = "qa/CHAPTER10_BROWSER_QA.json"
    browser_qa = read_json(browser_relative)
    require_pass(browser_qa, "Chapter 10 browser QA")
    if browser_qa.get("failures") != []:
        raise RuntimeError("Chapter 10 browser QA records failures")
    assert_path("Chapter 10 browser surface", browser_qa.get("surface"), HTML_ROOT)
    if browser_qa.get("canonical_html_manifest_sha256") != html_manifest.get("canonical_manifest_sha256"):
        raise RuntimeError("Chapter 10 browser QA targets a different HTML tree")
    browser_evidence = validate_reported_evidence(browser_qa, "Chapter 10 browser evidence")
    browser = {
        "status": "pass",
    ...
    require_pass(docs_manifest, "Chapter 10 docs manifest")
    require_pass(docs_qa, "Chapter 10 docs QA")
    if docs_manifest.get("failures") not in (None, []) or docs_qa.get("failures") not in (None, []):
        raise RuntimeError("Chapter 10 docs evidence records failures")
    docs_receipts = {
    ...
    for key, report in reports.items():
        require_pass(report, f"Chapter 10 PDF {key}")
    pdf_row = file_row(PDF_RELATIVE)
    assert_identity("Chapter 10 PDF run 1", reports["deterministic_run_1"], pdf_row)
    assert_identity("Chapter 10 PDF run 2", reports["deterministic_run_2"], pdf_row)
    structure_artifact = require_dict(reports["structure_qa"].get("artifact"), "PDF structure artifact")
    assert_path("Chapter 10 PDF structure artifact", structure_artifact.get("path"), PDF_RELATIVE)
    assert_identity("Chapter 10 PDF structure artifact", structure_artifact, pdf_row)
    visual_pdf = require_dict(reports["visual_qa"].get("pdf"), "PDF visual artifact")
    assert_path("Chapter 10 PDF visual artifact", visual_pdf.get("path"), PDF_RELATIVE)
    assert_identity("Chapter 10 PDF visual artifact", visual_pdf, pdf_row)
    if reports["visual_qa"].get("sha256") != pdf_row["sha256"]:
        raise RuntimeError("Chapter 10 PDF visual-QA top-level hash is stale")
    pages = reports["visual_qa"].get("pages")
    if not isinstance(pages, int) or pages <= 0:
        raise RuntimeError("Chapter 10 PDF visual QA has no valid page count")
    pdf = {
        **pdf_row,
        "status": "pass",
        "pages": pages,
    ...
    if len(all_source_names) != EXPECTED_TOTAL_SOURCE_FILES or len(set(all_source_names)) != EXPECTED_TOTAL_SOURCE_FILES:
        raise RuntimeError(
            f"cumulative Chapters 1-10 source closure is not exactly "
    ...
    chapter_10 = {
        ...
        "files": chapter_files,
        "source_qa": "repo/qa/CHAPTER10_SOURCE_QA.json",
        **dict(source_counts),
    ...
    prior_publication_receipts = {
        Path(relative).stem.removeprefix("CHAPTER09_").removesuffix("_PUBLICATION_RECEIPT").lower(): file_row(relative)
    ...
    if companion_assets.get("images") != []:
        raise RuntimeError("Chapter 10 companion QA unexpectedly declares images")
    if companion_assets.get("interactive_or_remote_surfaces") != []:
        raise RuntimeError("Chapter 10 companion QA unexpectedly declares remote surfaces")
    backend_assets = require_list(backend.get("assets"), "backend assets")
    if backend_assets:
        raise RuntimeError("Chapter 10 backend unexpectedly declares active assets")
    for asset in backend_assets:
        ...
        for raw_format in require_list(asset_row.get("formats"), "backend asset formats"):
            ...
            current = file_row(relative)
            assert_identity(f"Chapter 10 asset {relative}", format_row, current)
    ...
    if correction_statuses != Counter({"verified": EXPECTED_CORRECTION_RECORDS}):
        raise RuntimeError(
            f"Chapter 10 correction closure is not exactly "
            f"{EXPECTED_CORRECTION_RECORDS} verified records: "
    ...
    caveats = [
        ...
        "Whole-book figure provenance remains a final-release gate; Chapter 10 introduces no active assets.",
    ]
    ...
    qa_receipts = {
        ...
        "browser": file_row(browser_relative),
    ...
    package_inventory_rows = [
        ...
        *[control_row(relative) for relative in (*CONTROL_INPUTS, TERMINOLOGY_AUDIT)],
        ...
        *[file_row(f"qa/CHAPTER{number:02d}_SOURCE_MANIFEST.json") for number in range(1, 10)],
        source_qa_row,
        companion_qa_row,
    ...
    manifest = {
        ...
        "lane": "O003/C90",
        "locale": "id-ID",
        "boundary": "chapters_01_10_with_separately_licensed_self_study_companions",
        "status": manifest_status,
        "pending_evidence": [],
        "prior_admission": {
            **file_row(PRIOR_MANIFEST),
            "admission_basis": "locally validated and publicly read-back Chapter 9 source manifest",
        ...
        "authority": {
            ...
            "chapter_10_ordered_sha256": authority_combined,
            "chapter_10_raw_concatenated_sha256": authority_raw,
        },
        ...
        "chapter_10_identity_bundle": {
            ...
            "cumulative_reader_wrapper": file_row("source/chapters_01_10_reader.ptx"),
    ...
    print(
        json.dumps(
            {
                "status": manifest_status,
                "output": "repo/qa/CHAPTER10_SOURCE_MANIFEST.json",
                "bytes": OUTPUT.stat().st_size,
                "sha256": sha256(OUTPUT),
                "chapter_10_combined_sha256": chapter_combined,
                "cumulative_combined_sha256": cumulative_combined,
