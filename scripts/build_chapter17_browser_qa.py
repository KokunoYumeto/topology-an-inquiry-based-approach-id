#!/usr/bin/env python3
"""Run deterministic responsive-browser QA for the cumulative Chapter 17 reader.
...
viewports and the Chapter 17 source/companion surfaces, and exercises one
complete Chapter 17 staged disclosure.  It does not build or modify the reader.
...
DEFAULT_SURFACE = ROOT / "output/chapters01-17-html"
DEFAULT_OUTPUT = ROOT / "qa/CHAPTER17_BROWSER_QA.json"
VIEWPORTS = ((1440, 900), (1280, 900), (1024, 900), (768, 900), (390, 900))

ROOT_PAGE = "frontmatter-1.html"
BOOK_PAGE = "o003-c90-chapters-01-17-reader.html"
CHAPTER_PAGE = "chap_Compact_topology.html"
EXERCISES_PAGE = "sec_compact_top_exer.html"
COMPANION_PAGE = "o003-c90-ch17-companion.html"
GUIDES_PAGE = "o003-c90-ch17-source-guides-a.html"
MASTERY_PAGE = "o003-c90-ch17-mastery.html"
EDITION_NOTE_PAGE = "o003-c90-ch17-edition-note.html"
GUIDE_ID = "o003-c90-ch17-guide-01"

EVIDENCE_PATHS = (
    ROOT / "qa/CHAPTER17_HTML_MANIFEST.json",
    ROOT / "qa/CHAPTER17_HTML_QA.json",
    ROOT / "qa/CHAPTER17_COMPANION_QA.json",
)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Already-built cumulative Chapter 17 HTML directory.",
    )
    parser.add_argument(
        "--output",
        type=Path,
...
def inspect_identity(page: Any) -> dict[str, Any]:
    """Collect Chapter 17 boundary identity and navigation facts."""
    return page.evaluate(
        ...
        () => {
          ...
          return {
            ...
            rootLinkCount: countHref('o003-c90-chapters-01-17-reader.html'),
            chapterLinkCount: countHref('chap_Compact_topology.html'),
            exercisesLinkCount: countHref('sec_compact_top_exer.html'),
            companionLinkCount: countHref('o003-c90-ch17-companion.html'),
            masteryLinkCount: countHref('o003-c90-ch17-mastery.html'),
            editionNoteLinkCount: countHref('o003-c90-ch17-edition-note.html'),
            legacyBoundaryLinks: allHrefs.filter((href) =>
              href.includes('chapters-01-16-reader') ||
              href.includes('ch16-edition-note')
            ).length
          };
        }
        """
...
def validate_identity(
    ...
    checks = {
        ...
        "root_heading_names_chapters_01_17": "Bab 1-17" in root_heading,
        "root_exposes_chapter_17": int(root.get("chapterLinkCount") or 0) > 0,
        ...
        "no_chapter_16_boundary_links": int(root.get("legacyBoundaryLinks") or 0) == 0,
        "chapter_language_id_ID": chapter.get("language") == "id-ID",
        "chapter_book_link_exact": chapter.get("bookLink") == BOOK_PAGE,
        "chapter_heading_is_chapter_17": "Bab 17" in chapter_heading,
        "chapter_heading_names_compact_spaces": "ruang kompak" in chapter_heading.lower(),
    ...
    engine.add_failed_checks(failures, "Chapter 17 identity/navigation", checks)
    return {
...
def static_closure(failures: list[str]) -> dict[str, Any]:
    path = ROOT / "qa/CHAPTER17_COMPANION_QA.json"
    payload = engine.load_json(path, failures)
    counts = payload.get("counts") if isinstance(payload.get("counts"), dict) else {}
    observed = {
        "source_prompt_entries": counts.get("canonical_source_entries"),
        "mastery_entries": counts.get("mastery_entries"),
        "staged_surfaces": counts.get("staged_surfaces"),
        "grouping_nodes": counts.get("grouping_nodes"),
    }
    expected = {
        "source_prompt_entries": 75,
        "mastery_entries": 8,
        "staged_surfaces": 332,
        "grouping_nodes": 2,
    }
    ...
    if not passed:
        failures.append(
            f"Chapter 17 static companion closure mismatch: observed {observed}, "
            f"expected {expected}"
        )
    return {
        **observed,
        "source": "qa/CHAPTER17_COMPANION_QA.json and generated companion HTML controls",
        "status": "pass" if passed else "fail",
    }


def evidence_records(failures: list[str]) -> tuple[list[dict[str, Any]], str | None]:
    """Bind the exact Chapter 17 manifest and prerequisite QA receipts."""
    ...
    for path in EVIDENCE_PATHS:
        ...
        payload = engine.load_json(path, failures)
        if path.name == "CHAPTER17_HTML_MANIFEST.json":
            canonical_manifest_sha = payload.get("canonical_manifest_sha256")
            if not canonical_manifest_sha:
                failures.append(
                    "CHAPTER17_HTML_MANIFEST.json lacks canonical_manifest_sha256"
                )
...
def configure_engine() -> None:
    """Bind the maintained Chapter 14 browser engine helpers to Chapter 17."""
    engine.ROOT = ROOT
        ...
        receipt.update(
            {
                ...
                "surface_regression": {
                    ...
                    "chapter_17_desktop": chapter_row,
                    "chapter_17_exercises_tablet": exercises_row,
                    "chapter_17_companion_mobile": companion_row,
                    "chapter_17_mastery_mobile": mastery_row,
                    "all_geometry_checks_pass": all(
                        row.get("status") == "pass" for row in geometry_rows
                    ),
                },
                "chapter_17_semantics": semantics,
                "staged_disclosures": {
                    "chapter_17_live_interaction": disclosure,
                ...
                "checks": {
                    ...
                    "chapter_17_navigation_and_identity": semantics["status"],
