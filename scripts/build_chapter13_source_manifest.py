#!/usr/bin/env python3
"""Build the fail-closed cumulative Chapters 1-13 admission manifest.

Run only after the Chapter 13 source, final companion/backend, cumulative
...
OUTPUT = ROOT / "qa/CHAPTER13_SOURCE_MANIFEST.json"
MODEL = "OpenAI Codex gpt-5.6-sol, Ultra"

PRIOR_MANIFEST = "qa/CHAPTER12_SOURCE_MANIFEST.json"
PRIOR_MANIFEST_IDENTITY = {
    "bytes": 129_429,
    "sha256": "2ca4971f420885f8fafd07f64801ecf894d02acdfca74d52396d5841b8a43279",
}
BOUNDARY = "chapters_01_13_with_separately_licensed_self_study_companions"
PRIOR_BOUNDARY = "chapters_01_12_with_separately_licensed_self_study_companions"

CHAPTER_SOURCE_FILES = (
    "chap_Closed_sets_topology.ptx",
    "sec_closed_sets_top_intro.ptx",
    "sec_union_inter_closed.ptx",
    "sec_limit_points.ptx",
    "sec_top_closure.ptx",
    "sec_set_boundary.ptx",
    "sec_separation_ax.ptx",
    "sec_closed_sets_top_summ.ptx",
    "sec_closed_sets_top_exer.ptx",
)
COMPANION_FILES = (
    "companion/chapter_13_source_guides_a.ptx",
    "companion/chapter_13_source_guides_b.ptx",
    "companion/chapter_13_source_guides_c.ptx",
    "companion/chapter_13_source_guides_d.ptx",
    "companion/chapter_13_source_guides_e.ptx",
    "companion/chapter_13_source_guides_f.ptx",
    "companion/chapter_13_exercise_guides_a.ptx",
    "companion/chapter_13_exercise_guides_b.ptx",
    "companion/chapter_13_exercise_guides_c.ptx",
    "companion/chapter_13_exercise_guides_d.ptx",
    "companion/chapter_13_exercise_guides_e.ptx",
    "companion/chapter_13_exercise_guides_f.ptx",
    "companion/chapter_13_mastery.ptx",
)
COMPANION_WRAPPER = "companion/chapter_13_closed_sets_topological_spaces_self_study.ptx"
READER_WRAPPER = "source/chapters_01_13_reader.ptx"
FINAL_BACKEND = "backend/chapter_13_companion_manifest.json"
FINAL_ALIASES = "backend/chapter_13_entry_aliases.csv"
PROMPT_MAP = "backend/chapter_13_source_prompt_map.csv"
GROUPING_BACKEND = "backend/chapter_13_grouping_nodes.json"

SOURCE_QA = "qa/CHAPTER13_SOURCE_QA.json"
COMPANION_QA = "qa/CHAPTER13_COMPANION_QA.json"
SCHEMA_QA = "qa/CHAPTER13_CUMULATIVE_SCHEMA_QA.json"
HTML_MANIFEST = "qa/CHAPTER13_HTML_MANIFEST.json"
HTML_RUN_1 = "qa/CHAPTER13_HTML_MANIFEST_RUN1.json"
HTML_RUN_2 = "qa/CHAPTER13_HTML_MANIFEST_RUN2.json"
HTML_QA = "qa/CHAPTER13_HTML_QA.json"
BROWSER_QA = "qa/CHAPTER13_BROWSER_QA.json"
ASSET_PDF_QA = "qa/CHAPTER13_ASSET_PDF_METADATA_QA.json"
PDF_RECEIPTS = {
    "deterministic_run_1": "qa/CHAPTER13_PDF_RUN1_HASH.json",
    "deterministic_run_2": "qa/CHAPTER13_PDF_RUN2_HASH.json",
    "structure_qa": "qa/CHAPTER13_PDF_STRUCTURE.json",
    "visual_qa": "qa/CHAPTER13_PDF_VISUAL_QA.json",
}
DOCS_RECEIPTS = (
    "qa/CHAPTER13_DOCS_MANIFEST.json",
    "qa/CHAPTER13_DOCS_QA.json",
    "qa/CHAPTER13_BUILD_QA.md",
)
HTML_ROOT = "output/chapters01-13-html"
PDF_PATH = "output/chapters01-13-pdf/chapters_01_13_reader.pdf"
...
CONTROL_FILES = (
    ...
    "00_control/CHAPTER13_TERMINOLOGY_AUDIT.md",
)
...
def validate_html(manifest: dict[str, Any]) -> tuple[int, int, int]:
    ...
    if set(actual) != set(expected):
        raise RuntimeError("cumulative Chapter 13 HTML closure differs from its manifest")
    total = 0
    html_files = 0
    ...
    if facts != (manifest.get("file_count"), manifest.get("html_files"), manifest.get("total_bytes")):
        raise RuntimeError("cumulative Chapter 13 HTML census differs")
    return facts
...
def validate_final_companion() -> tuple[dict[str, Any], dict[str, Any], list[dict[str, object]]]:
    ...
    if backend.get("partial") is not False or backend.get("status") != expected_status:
        raise RuntimeError("Chapter 13 backend is partial or has not reached final companion status")
    if companion_qa.get("status") != "pass" or companion_qa.get("failures") != []:
        raise RuntimeError("Chapter 13 companion QA is not passing")
    ...
    expected_correction_ids = [f"O003-C{number:03d}" for number in range(146, 162)]
    if corrections.get("ids") != expected_correction_ids:
        raise RuntimeError("Chapter 13 correction-ID closure changed")
    coverage = require_dict(backend.get("coverage"), "backend coverage")
    expected_coverage = {
        "covered_source_prompt_guides": 76,
        "pending_source_prompt_guides": 0,
        "covered_mastery_checks": 8,
        "entry_total": 84,
        "staged_surface_total": 336,
        "grouping_nodes": 7,
    }
    for key, expected in expected_coverage.items():
        if coverage.get(key) != expected:
            raise RuntimeError(f"Chapter 13 backend coverage changed: {key}")
    ...
    if missing_includes:
        raise RuntimeError(f"Chapter 13 wrapper omits final companion files: {missing_includes}")
...
def build_manifest() -> dict[str, Any]:
    assert_identity("pinned Chapter 12 manifest", PRIOR_MANIFEST_IDENTITY, identity(ROOT / PRIOR_MANIFEST))
    prior = read_json(PRIOR_MANIFEST)
    require_pass(prior, "Chapter 12 source manifest")
    if prior.get("boundary") != PRIOR_BOUNDARY or prior.get("pending_evidence") != []:
        raise RuntimeError("unexpected Chapter 12 admission boundary")
    ...
    if len(prior_names) != 81 or len(set(prior_names)) != 81:
        raise RuntimeError("prior translated source closure is not exactly 81 files")

    source_qa = read_json(SOURCE_QA)
    require_pass(source_qa, "Chapter 13 source QA")
    if source_qa.get("failures") != []:
        raise RuntimeError("Chapter 13 source QA records failures")
    qa_sources: dict[str, dict[str, Any]] = {}
    for raw in require_list(source_qa.get("files"), "Chapter 13 source QA files"):
        row = require_dict(raw, "Chapter 13 source QA row")
        name = row.get("file")
        if not isinstance(name, str) or not name or "/" in name or "\\" in name:
            raise RuntimeError(f"invalid Chapter 13 source-QA filename: {name!r}")
        if name in qa_sources:
            raise RuntimeError(f"duplicate Chapter 13 source-QA filename: {name}")
        qa_sources[name] = row
    if set(qa_sources) != set(CHAPTER_SOURCE_FILES):
        raise RuntimeError("Chapter 13 source QA closure is not the exact nine-file target")
    ...
    if source_qa.get("combined_translated_sha256") != chapter_combined:
        raise RuntimeError("Chapter 13 translated combined identity is stale")

    backend, companion_qa, companion_rows = validate_final_companion()
    reader_text = (ROOT / READER_WRAPPER).read_text(encoding="utf-8")
    if MODEL not in reader_text or "chapter_13_closed_sets_topological_spaces_self_study.ptx" not in reader_text:
        raise RuntimeError("Chapter 13 reader wrapper lacks exact provenance or companion include")
    project_text = (ROOT / "project.ptx").read_text(encoding="utf-8")
    required_target_fragments = (
        '<target name="chapters01-13-html">',
        '<target name="chapters01-13-pdf" pdf-method="xelatex">',
        '<source>source/chapters_01_13_reader.ptx</source>',
        '<output-dir>output/chapters01-13-html</output-dir>',
        '<output-dir>output/chapters01-13-pdf</output-dir>',
    )
    if any(fragment not in project_text for fragment in required_target_fragments):
        raise RuntimeError("project.ptx lacks the exact cumulative Chapter 13 wrapper/targets")

    schema_qa = read_json(SCHEMA_QA)
    require_pass(schema_qa, "Chapter 13 cumulative schema QA")
    html_manifest = read_json(HTML_MANIFEST)
    html_qa = read_json(HTML_QA)
    browser_qa = read_json(BROWSER_QA)
    require_pass(html_qa, "Chapter 13 HTML QA")
    require_pass(browser_qa, "Chapter 13 browser QA")
    ...
    for key, value in zip(("file_count", "html_files", "total_bytes"), html_facts):
        if html_qa.get(key) != value:
            raise RuntimeError(f"Chapter 13 HTML QA differs for {key}")
    if html_qa.get("canonical_manifest_sha256") != html_manifest.get("canonical_manifest_sha256"):
        raise RuntimeError("Chapter 13 HTML canonical identity differs")

    pdf_visual = read_json(PDF_RECEIPTS["visual_qa"])
    require_pass(pdf_visual, "Chapter 13 PDF visual QA")
    pdf_row = repo_row(PDF_PATH)
    assert_identity("Chapter 13 PDF", pdf_visual.get("pdf"), pdf_row)
    pdf_pages = pdf_visual.get("pages")
    if not isinstance(pdf_pages, int) or pdf_pages <= 0:
        raise RuntimeError("Chapter 13 PDF visual QA has no positive page count")
    qa_paths = [
    ...
    translated["chapter_13"] = chapter_13
    ...
    manifest = {
        ...
        "cumulative_component_identities": {
            "translated_source_chapters_01_13": {
                "file_count": 90,
                "chapters": [f"chapter_{number:02d}" for number in range(1, 14)],
                "cumulative_combined_sha256": translated["cumulative_combined_sha256"],
            },
            "companions_chapters_01_13": cumulative_companions,
            "backends_chapters_01_13": cumulative_backends,
        },
        "companion_coverage": {
            "source_prompt_guides": 76,
            "mastery_checks": 8,
            "total_entries": 84,
            "surfaces_per_entry": ["statement", "hint", "answer", "solution"],
            "total_surfaces": 336,
            "grouping_nodes_mapped": 7,
        ...
        "known_caveats": [
            "This is a complete admitted 13-of-20 cumulative reader boundary, not the complete edition.",
    ...
    if (
        ...
        raise RuntimeError("Chapter 13 checkpoint scope is not represented truthfully")
    return manifest
...
def main() -> int:
    ...
    try:
        ...
        raise SystemExit(f"Chapter 13 admission manifest gate failed: {exc}") from exc
    summary = {
    ...
    if not args.check_only:
        ...
        summary.update({"output": "repo/qa/CHAPTER13_SOURCE_MANIFEST.json", **identity(OUTPUT)})
