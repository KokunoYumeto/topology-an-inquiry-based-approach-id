#!/usr/bin/env python3
"""Validate Chapter 14 in the admitted cumulative book context.

The Chapter 14 source is intentionally not yet added to the public Chapters
1--13 wrapper.  PreTeXt's RelaxNG start pattern does not validate a standalone
chapter in the same way as a chapter inside a book, so this checker inserts the
single local XInclude in memory, expands the complete local closure, and
validates the resulting cumulative tree without creating a misleading reader.
"""
...
def chapter_closure(entry: Path, repo: Path) -> list[Path]:
    ...
    while pending:
        ...
        if not current.is_file() or repo.resolve() not in (current, *current.parents):
            raise RuntimeError(f"invalid local Chapter 14 include: {current}")
        seen.add(current)
        ...
        for node in tree.iter(XI):
            ...
            if not href or "://" in href or href.startswith(("/", "\\")):
                raise RuntimeError(f"nonlocal Chapter 14 include in {current}: {href}")
...
def main() -> int:
    ...
    base = repo / "source" / "chapters_01_13_reader.ptx"
    chapter = repo / "source" / "chap_continuity_topology.ptx"
    ...
    namespace = {"xi": XI_NS}
    prior = tree.xpath(
        '//xi:include[@href="./chap_Closed_sets_topology.ptx"]',
        namespaces=namespace,
    )
    if len(prior) != 1:
        raise RuntimeError("could not identify the unique admitted Chapter 13 include")
    if tree.xpath(
        '//xi:include[@href="./chap_continuity_topology.ptx"]',
        namespaces=namespace,
    ):
        raise RuntimeError("base wrapper already includes Chapter 14")

    include = etree.Element(XI)
    include.set("href", "./chap_continuity_topology.ptx")
    ...
    report = {
        ...
        "validation_context": (
            "Chapter 14 inserted in memory immediately after Chapter 13 in "
            "the admitted Chapters 1-13 book wrapper"
        ),
        "base_wrapper": identity(base, base.relative_to(repo).as_posix()),
        "inserted_include": "./chap_continuity_topology.ptx",
        "chapter_closure": [
